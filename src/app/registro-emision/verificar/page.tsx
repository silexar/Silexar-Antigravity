...
const NeuromorphicCard = ({ children, className = '' }: { children: React.ReactNode, className?: string }) => (
  <div className={`bg-[#dfeaff] rounded-2xl shadow-[9px_9px_16px_#bec8de,-9px_-9px_16px_#ffffff] ${className}`}>
    {children}
  </div>
);
...
const NeuromorphicButton = ({ children, onClick, className = '', active = false }: { children: React.ReactNode, onClick?: () => void, className?: string, active?: boolean }) => (
  <button
    ...
    className={`
      ...
      ${active
        ? 'bg-[#dfeaff] text-[#6888ff] shadow-[inset_6px_6px_12px_#bec8de,inset_-6px_-6px_12px_#ffffff]'
        : 'bg-[#dfeaff] text-[#69738c] shadow-[6px_6px_10px_#bec8de,-6px_-6px_10px_#ffffff] hover:shadow-[3px_3px_6px_#bec8de,-3px_-3px_6px_#ffffff] hover:-translate-y-0.5 active:shadow-[inset_4px_4px_8px_#bec8de,inset_-4px_-4px_8px_#ffffff] active:translate-y-0'
      }
      ${className}
    `}
  >
...
export default function VerificadorPage() {
  ...
  const handleAICommand = (cmd: string) => {
    console.log('[DEBUG] handleAICommand called with:', cmd);
    const cmdLower = cmd.toLowerCase();

    // Start verification for any command that implies verification action
    if (cmdLower.includes('verificar') ||
      cmdLower.includes('nueva') ||
      cmdLower.includes('auditar') ||
      cmdLower.includes('buscar') ||
      cmdLower.includes('status') ||
      cmdLower.includes('express')) {
      console.log('[DEBUG] Command matched, starting verification');
      const materialesMock: Material[] = [
        ...
        { id: '3', nombre: 'Banner App Falabella', tipo: 'display_banner', duracion: 0, selected: true },
      ];
  ...
  if (viewState === 'wizard') {
    return (
      <div className="min-h-screen bg-[#dfeaff] p-8">
        ...
        onAddToBasket={() => {
          if (singleResult) {
            setBasketItems(prev => [...prev, {
              id: uuidv4(),
              verificacionId: uuidv4(),
              materialId: singleResult.materialId || 'unknown',
              materialNombre: singleResult.nombreMaterial,
              tipoMaterial: singleResult.tipoMaterial || 'audio_pregrabado',
              spxCode: 'SPX-' + (singleResult.materialId || '000'),
              hora: singleResult.horaEmision || '00:00',
              emissora: singleResult.emisora || 'Desconocida',
              clipUrl: singleResult.clipUrl,
              seleccionada: true
            }]);
  ...
  return (
    <main className="min-h-screen bg-[#dfeaff] text-[#69738c] p-8 font-sans relative">

      {/* EVIDENCE BASKET FLOATING COMPONENT */}
      <EvidenceBasket
        items={basketItems}
        onRemoveItem={(id: string) => setBasketItems(prev => prev.filter(i => i.id !== id))}
        onToggleItem={(id: string) => setBasketItems(prev => prev.map(i => i.id === id ? { ...i, seleccionada: !i.seleccionada } : i))}
        onClearAll={() => setBasketItems([])}
        onExportSelected={() => alert('Exportar seleccionados (demo)')}
        onGenerateSecureLink={() => alert('Generar link seguro (demo)')}
        isExpanded={false}
        onToggleExpand={() => { }}
      />

      {/* SECURITY DEMO OVERLAY */}
      <div className="fixed top-24 right-6 z-[100] bg-[#dfeaff] text-white p-3 rounded-lg shadow-2xl opacity-90 hover:opacity-100 transition-opacity scale-90 origin-top-right group">
        <div className="text-[9px] font-bold uppercase tracking-widest mb-1 text-[#9aa3b8] group-hover:text-[#6888ff]">Security / RBAC Simulator</div>
        <select
          value={currentRole}
        ...
        <div className="mt-1 flex items-center justify-between text-[8px]">
          <span className="flex items-center gap-1 text-[#6888ff]"><div className="w-1 h-1 rounded-full bg-[#6888ff] animate-pulse" /> Zero-Trust</span>
          <span className="text-[#69738c] font-mono">ID: {Math.random().toString(36).substr(2, 4)}</span>
        </div>
      </div>
      ...
      <header className="mb-10">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-6 items-center">
            <SentinelStatus />
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#dfeaff] shadow-[inset_3px_3px_6px_#bec8de,inset_-3px_-3px_6px_#ffffff]">
              <div className="w-3 h-3 rounded-full bg-[#6888ff] animate-pulse"></div>
              <span className="text-xs font-bold text-[#69738c]">SYSTEM ONLINE</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          />
          <MetricCard
            icon={<BarChart3 className="text-[#6888ff]" />}
            label="Cumplimiento"
            value="98.5%"
      ...
      <section className="mb-12 relative z-40">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-center gap-3 mt-4">
            <SuggestionPill text="Verificar Coca-Cola Ayer" onClick={() => handleAICommand('Verificar Coca-Cola Ayer')} />
            <SuggestionPill text="Auditar Banner Banco Falabella" onClick={() => handleAICommand('Auditar Banner Banco Falabella')} />
            <SuggestionPill text="Status Digital 360" onClick={() => handleAICommand('Status Digital 360')} />
          </div>
        </div>
      </section>
      ...
      <section className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-[#dfeaff] rounded-2xl shadow-[inset_4px_4px_8px_#bec8de,inset_-4px_-4px_8px_#ffffff] p-5">
...
function MetricCard({ icon, label, value, subtext }: MetricCardProps) {
  return (
    <NeuromorphicCard className="p-6 flex items-center justify-between">
      <div>
        <p className="text-[#69738c] text-xs font-bold uppercase mb-1 tracking-wider">{label}</p>
        <div className="text-3xl font-black text-slate-700">{value}</div>
        <p className="text-xs text-[#6888ff] font-medium mt-1">{subtext}</p>
      </div>
      <div className="p-3 rounded-xl bg-[#dfeaff] shadow-[inset_5px_5px_10px_#bec8de,inset_-5px_-5px_10px_#ffffff]">
        {icon}
      </div>
    </NeuromorphicCard>
  );
...
function ActionButton({ icon, label, desc, onClick }: ActionButtonProps) {
  return (
    <button
      onClick={onClick}
      className="group bg-[#dfeaff] p-6 rounded-2xl transition-all duration-300 flex flex-col items-center text-center gap-4
        shadow-[9px_9px_16px_#bec8de,-9px_-9px_16px_#ffffff]
        hover:shadow-[5px_5px_8px_#bec8de,-5px_-5px_8px_#ffffff]
        hover:-translate-y-1
        active:shadow-[inset_4px_4px_8px_#bec8de,inset_-4px_-4px_8px_#ffffff]
        active:translate-y-0
      "
    >
      <div className="p-4 rounded-full bg-[#dfeaff] shadow-[inset_5px_5px_10px_#bec8de,inset_-5px_-5px_10px_#ffffff] group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div>
...
const SuggestionPill = ({ text, onClick }: { text: string, onClick?: () => void }) => (
  <button
    onClick={onClick}
    className="px-3 py-1.5 rounded-full bg-[#dfeaff] border border-[#bec8de] text-xs text-[#69738c] font-bold hover:bg-[#d0e0ff] hover:scale-105 active:scale-95 transition-all"
  >
    {text}
  </button>
);
