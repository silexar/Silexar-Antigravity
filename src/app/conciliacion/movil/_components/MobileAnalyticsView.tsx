    ...
    <div className="space-y-6 pb-24 animate-in slide-in-from-right-10 duration-500">
      ...
      <header className="flex justify-between items-center bg-[#F0EDE8] -mx-4 px-4 py-3 border-b border-white/5 sticky top-[-16px] z-[60]">
         ...
         <div className="text-center">
            ...
            <div className="text-[10px] text-[#888780] font-bold uppercase tracking-widest">Últimos 30 Días</div>
         </div>
      ...
      <div className="grid grid-cols-3 gap-2">
         {metrics.map((m) => (
            <div key={m.l} className="bg-[#F0EDE8] border border-white/5 p-3 rounded-2xl text-center">
               <div className="text-[8px] font-black text-[#888780] uppercase mb-1">{m.l}</div>
      ...
      <div className="space-y-3">
         <h3 className="text-[10px] font-black text-[#2C2C2A] uppercase tracking-widest pl-1">Performance por Emisora</h3>
         {[
           { n: 'Corazón', p: '99.1%', s: '🥇', c: 'text-emerald-400' },
           { n: 'Play FM', p: '98.2%', s: '🥈', c: 'text-amber-400' },
           { n: 'Sonar FM', p: '97.8%', s: '🥉', c: 'text-blue-400' }
         ].map((e) => (
            <div key={e.n} className="bg-[#F0EDE8] border border-white/5 p-4 rounded-2xl flex items-center justify-between">
               <div className="flex items-center gap-3">
                  <span className="text-xl">{e.s}</span>
                  <span className="text-xs font-bold text-[#2C2C2A]">{e.n}</span>
               </div>
      ...
      <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-3xl p-5 space-y-4">
         ...
         <div className="bg-[#F5F2EE]/50 p-3 rounded-xl border border-white/5">
            <div className="text-[9px] font-bold text-amber-500 uppercase mb-1">Patrón Crítico</div>
            <p className="text-[10px] text-[#5F5E5A] leading-relaxed italic">"Fallas recurrentes en Sonar FM viernes 14h-16h detectadas."</p>
         </div>
         <button className="w-full bg-indigo-600 text-[#2C2C2A] py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">VER PLAN DE MEJORA</button>
      </div>

       {/* CAUSAS RAÍZ MÓVIL SIMPLIFICADO */}
       <div className="bg-[#F0EDE8] p-5 rounded-3xl border border-white/5 space-y-4">
          <h3 className="text-[10px] font-black text-[#888780] uppercase tracking-widest">Distribución de Fallas</h3>
          <div className="space-y-3">
             {[
               ...
               { l: 'Humanos/Ed.', p: '23%', c: 'bg-red-500' }
                ...
                <div key={c.l} className="space-y-1">
                   <div className="flex justify-between text-[9px] font-bold text-[#5F5E5A]">
                      <span>{c.l}</span>
                      <span>{c.p}</span>
                   </div>
