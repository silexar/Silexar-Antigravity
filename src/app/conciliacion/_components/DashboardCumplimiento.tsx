    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="flex justify-between items-center bg-slate-800/50 p-6 rounded-2xl border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            <h2 className="text-2xl font-black text-white uppercase tracking-tighter">📊 Analytics Cumplimiento Emisión</h2>
            <p className="text-xs text-indigo-400 font-black tracking-widest uppercase">Últimos 30 Días • Vista Ejecutiva TIER 0</p>
          </div>
        </div>
        <div className="flex gap-3 relative z-10">
           <button className="bg-slate-900 border border-slate-700 text-slate-300 px-6 py-2.5 rounded-xl text-xs font-black hover:bg-slate-800 transition-all">EXPORTAR DATA</button>
           <button className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-xs font-black shadow-xl shadow-indigo-600/20 hover:scale-105 transition-all">PROGRAMAR REPORTE</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
         {[
           ...
           { label: 'Intervención Manual', val: '5.7%', sub: 'Requerido por Traffic', color: 'text-amber-400' }
            ...
            <div key={i} className="bg-slate-900/80 border border-white/5 p-5 rounded-2xl shadow-xl hover:border-indigo-500/30 transition-all group">
               <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 group-hover:text-indigo-400 transition-colors">{m.label}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         ...
         <div className="lg:col-span-2 bg-slate-900/50 border border-slate-800 rounded-3xl p-6 shadow-2xl">
            ...
            <div className="space-y-4">
               {emisoras.map((e, idx) => (
                  <div key={idx} className="bg-slate-800/30 border border-white/5 rounded-2xl p-5 hover:bg-slate-800/50 transition-all flex items-center justify-between">
                     <div className="flex items-center gap-6">
                        <span className="text-3xl">{e.score}</span>
                        <div>
                           <div className="text-lg font-black text-white">{e.name}</div>
                           <div className="text-xs text-slate-500 font-bold uppercase tracking-tighter">Spots: {e.total}</div>
                        </div>
                     </div>
                     <div className="grid grid-cols-2 gap-8 text-right">
                        <div>
                           <div className="text-sm font-black text-emerald-400">{e.metrics}</div>
                           <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Cumplimiento</div>
                        </div>
                        <div>
         ...
         <div className="space-y-6">
            <div className="bg-indigo-600/10 border border-indigo-500/30 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
               ...
               <div className="space-y-4 relative z-10">
                  <div className="bg-slate-900/80 p-4 rounded-2xl border-l-4 border-amber-500 shadow-lg">
                     <div className="text-[10px] font-bold text-amber-500 mb-1">PATRÓN DETECTADO</div>
                     <p className="text-xs text-slate-200">Incremento de fallas en Sonar FM los viernes (14:00-16:00).</p>
                  </div>
                  <div className="bg-slate-900/80 p-4 rounded-2xl border-l-4 border-red-500 shadow-lg">
                     <div className="text-[10px] font-bold text-red-500 mb-1">EXCLUSIVIDAD RIESGO</div>
                     <p className="text-xs text-slate-200">Conflictos sector automotriz superan el umbral en 15%.</p>
                  </div>
                  <button className="w-full bg-indigo-600/20 border border-indigo-500/40 text-indigo-400 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-white/5 shadow-2xl">
               <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">📈 TENDENCIAS DETECTADAS</h3>
               <div className="space-y-3">
                  <div className="flex items-center gap-3">
                     ...
                     <div className="text-xs text-slate-300">Mejoría sostenida últimas 2 semanas.</div>
                  </div>
                  <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-400 text-xs font-black">▲</div>
                     <div className="text-xs text-slate-300">Reducción 40% fallas técnicas vs Q1.</div>
                  </div>
                  <div className="flex items-center gap-3">
                     ...
                     <div className="text-xs text-slate-300">+15% efectividad en recuperación IA.</div>
                  </div>
               </div>
            </div>
         </div>
