...
const neuro = {
  ...
  `,
  tabActive: `
    bg-gradient-to-br from-[#6888ff] to-[#5572ee] text-white
    shadow-[4px_4px_8px_#d1d5db,-4px_-4px_8px_#ffffff]
  `,
  tabInactive: `
    text-[#69738c] hover:text-[#69738c] hover:bg-[#dfeaff]
  `
};
...
const getTipoInfo = (tipo: TipoDocumento) => {
  const info: Record<TipoDocumento, { icon: React.ReactNode; color: string; label: string }> = {
    EMAIL: { icon: <Mail className="w-5 h-5" />, color: 'bg-[#dfeaff] text-[#9aa3b8]', label: 'Email' },
    COTIZACION: { icon: <FileText className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Cotización' },
    ORDEN_COMPRA: { icon: <File className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Orden de Compra' },
    VOUCHER: { icon: <File className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Voucher' },
    BRIEF: { icon: <FileText className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Brief' },
    PROPUESTA: { icon: <FileText className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Propuesta' },
    CONTRATO: { icon: <FileText className="w-5 h-5" />, color: 'bg-[#dfeaff] text-[#69738c]', label: 'Contrato' },
    FACTURA: { icon: <File className="w-5 h-5" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Factura' },
    ...
    OTRO: { icon: <File className="w-5 h-5" />, color: 'bg-[#dfeaff] text-[#69738c]', label: 'Otro' }
  };
  ...
  if (vista === 'grid') {
    return (
      ...
      >
        ...
        {/* Info */}
        <div>
          <p className="font-semibold text-[#9aa3b8] text-sm truncate mb-1" title={documento.nombre}>
            {documento.nombre}
          </p>
          
          <div className="flex items-center gap-2 text-xs text-[#69738c]">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-[#dfeaff]"
              >
                <Download className="w-4 h-4 text-[#69738c]" />
              </button>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-semibold text-[#9aa3b8] truncate" title={documento.nombre}>
            {documento.nombre}
          </p>
          {documento.soloLectura && <Lock className="w-3.5 h-3.5 text-[#9aa3b8]" />}
          {documento.procesadoPorIA && <Sparkles className="w-3.5 h-3.5 text-[#6888ff]" />}
        </div>
        
        <div className="flex items-center gap-3 mt-1 text-xs text-[#69738c]">
          ...
          {documento.version > 1 && (
            <>
              <span>•</span>
              <span className="text-[#6888ff]">v{documento.version}</span>
            </>
          )}
        </div>

        {documento.emailAsunto && (
          <p className="text-xs text-[#9aa3b8] mt-1 truncate">
          ...
          {showMenu && (
            <div className={`absolute right-0 top-full mt-2 ${neuro.panel} p-2 z-10 min-w-40`}>
              ...
              <button 
                ...
                className="w-full px-3 py-2 text-left text-sm hover:bg-[#dfeaff] text-[#9aa3b8] rounded-lg flex items-center gap-2"
              >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`${neuro.panel} p-6 mb-6`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className={`${neuro.badge} bg-[#6888ff]/10 text-[#6888ff] text-lg px-4 py-2`}>
          ...
          <div className="grid grid-cols-6 gap-4 mt-6">
            {(['NEGOCIACION', 'COMERCIAL', 'LEGAL', 'FACTURACION', 'OPERACIONES', 'MATERIAL_CREATIVO'] as CategoriaDocumento[]).map(cat => (
              ...
              >
                ...
                <p className="text-xs text-[#69738c] mt-1">{getCategoriaLabel(cat).replace(/\p{Emoji}/gu, '')}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`${neuro.panel} p-4 mb-6`}>
          <div className="flex items-center justify-between gap-4">
            ...
            {/* Vista */}
            <div className="flex items-center gap-1 bg-[#dfeaff] p-1 rounded-xl">
              <button
        ...
        {documentosFiltrados.length === 0 && (
          <div className={`${neuro.card} p-12 text-center`}>
            <FolderOpen className="w-16 h-16 text-[#9aa3b8] mx-auto mb-4" />
            <h3 className="text-xl font-bold text-[#69738c] mb-2">
              No hay documentos
            </h3>
            <p className="text-[#9aa3b8] mb-4">
