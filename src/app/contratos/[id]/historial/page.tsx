...
const getCategoriaInfo = (categoria: CategoriaAccion) => {
  const info: Record<CategoriaAccion, { icon: React.ReactNode; color: string; label: string }> = {
    CONTRATO: { icon: <FileText className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Contrato' },
    LINEA: { icon: <FileText className="w-4 h-4" />, color: 'bg-cyan-100 text-cyan-600', label: 'Línea' },
    APROBACION: { icon: <CheckCircle className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Aprobación' },
    DOCUMENTO: { icon: <FileText className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Documento' },
    COMUNICACION: { icon: <Mail className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Comunicación' },
    FIRMA: { icon: <PenTool className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Firma' },
    FACTURACION: { icon: <DollarSign className="w-4 h-4" />, color: 'bg-[#6888ff]/10 text-[#6888ff]', label: 'Facturación' },
    CONFIGURACION: { icon: <Monitor className="w-4 h-4" />, color: 'bg-[#dfeaff] text-[#69738c]', label: 'Configuración' },
    SEGURIDAD: { icon: <Shield className="w-4 h-4" />, color: 'bg-[#dfeaff] text-[#9aa3b8]', label: 'Seguridad' },
    SISTEMA: { icon: <RefreshCw className="w-4 h-4" />, color: 'bg-[#dfeaff] text-[#69738c]', label: 'Sistema' }
  };
...
const RegistroCard: React.FC<{
  ...
  return (
    ...
    >
      {/* Línea de conexión */}
      <div className="absolute left-6 top-10 bottom-0 w-0.5 bg-[#dfeaff]" />
      ...
      <div className={`${neuro.card} ml-12 p-4`}>
        ...
        >
          <div className="flex-1">
            ...
            <p className="font-semibold text-[#9aa3b8] mt-2">
              {registro.descripcion}
            </p>

            <div className="flex items-center gap-4 mt-2 text-xs text-[#69738c]">
          ...
          <div className="flex items-center gap-2">
            {registro.verificado && (
              <span className="text-[#6888ff]" title="Registro verificado">
                <Shield className="w-4 h-4" />
              </span>
            )}
            <Lock className="w-4 h-4 text-[#9aa3b8]" />
            <ChevronDown className={`w-5 h-5 text-[#9aa3b8] transition-transform ${expanded ? 'rotate-180' : ''}`} />
          </div>
        </div>

        {/* Cambios de valores */}
        {registro.cambios && registro.cambios.length > 0 && (
          <div className="mt-3 p-3 bg-[#dfeaff] rounded-xl">
            <p className="text-xs font-semibold text-[#69738c] mb-2">CAMBIOS REALIZADOS</p>
            {registro.cambios.map((cambio, idx) => (
              <div key={idx} className="flex items-center gap-3 text-sm">
                <span className="text-[#69738c]">{cambio.campo}:</span>
                <span className="text-[#9aa3b8] line-through">{String(cambio.valorAnterior)}</span>
                <ChevronRight className="w-4 h-4 text-[#9aa3b8]" />
                <span className="text-[#6888ff] font-semibold">{String(cambio.valorNuevo)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <AnimatePresence>
          {expanded && (
            <motion.div
              ...
              className="mt-4 pt-4 border-t border-[#bec8de] overflow-hidden"
            >
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                {/* Usuario */}
                <div>
                  <p className="text-xs text-[#9aa3b8] mb-1">Usuario</p>
                  <p className="font-medium text-[#69738c]">{registro.usuario.nombre}</p>
                  <p className="text-xs text-[#69738c]">{registro.usuario.email}</p>
                  <p className="text-xs text-[#9aa3b8]">{registro.usuario.rol}</p>
                </div>

                {/* Dispositivo */}
                <div>
                  <p className="text-xs text-[#9aa3b8] mb-1">Dispositivo</p>
                  <p className="font-medium text-[#69738c]">{registro.contexto.dispositivo}</p>
                  <p className="text-xs text-[#69738c]">{registro.contexto.navegador}</p>
                  <p className="text-xs text-[#9aa3b8]">{registro.contexto.sistemaOperativo}</p>
                </div>

                {/* Red */}
                <div>
                  <p className="text-xs text-[#9aa3b8] mb-1">Red</p>
                  <p className="font-mono text-xs text-[#69738c]">{registro.contexto.ip}</p>
                ...
                {/* Timestamp */}
                <div>
                  <p className="text-xs text-[#9aa3b8] mb-1">Timestamp</p>
                  <p className="font-mono text-xs text-[#69738c]">{registro.timestampUnix}</p>
                  <p className="text-xs text-[#69738c]">
              ...
              {/* Hashes de seguridad */}
              <div className="mt-4 p-3 bg-[#dfeaff] rounded-xl text-xs">
                <div className="flex items-center gap-2 text-[#6888ff] mb-2">
                  <Fingerprint className="w-4 h-4" />
                  <span className="font-semibold">FIRMA DIGITAL E INTEGRIDAD</span>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 text-[#9aa3b8] font-mono">
                  <div>
                    <span className="text-[#69738c]">Hash registro: </span>
                    <span className="text-[#6888ff]">{registro.hashRegistro}</span>
                  </div>
                  <div>
                    <span className="text-[#69738c]">Hash anterior: </span>
                    <span className="text-[#6888ff]">{registro.hashAnterior}</span>
                  </div>
                  <div className="lg:col-span-2">
                    <span className="text-[#69738c]">Firma digital: </span>
                    <span className="text-[#6888ff]">{registro.firmaDigital}</span>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`${neuro.panel} p-6 mb-6`}>
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              <div className="p-4 rounded-2xl bg-gradient-to-br from-[#bec8de] to-[#9aa3b8]">
                <History className="w-8 h-8 text-white" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-[#9aa3b8] flex items-center gap-2">
                  Historial del Contrato
                  <Lock className="w-5 h-5 text-[#9aa3b8]" />
                </h1>
          ...
          <div className="mt-4 p-4 bg-[#dfeaff] rounded-2xl flex items-center gap-4">
            <Lock className="w-8 h-8 text-[#6888ff]" />
            <div className="flex-1">
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          <div className={`${neuro.card} p-4 text-center`}>
            <p className="text-3xl font-bold text-[#9aa3b8]">{resumen.totalRegistros}</p>
            <p className="text-xs text-[#69738c]">Total registros</p>
          </div>
          <div className={`${neuro.card} p-4 text-center`}>
            <p className="text-3xl font-bold text-[#9aa3b8]">{resumen.usuariosUnicos}</p>
            <p className="text-xs text-[#69738c]">Usuarios</p>
          </div>
          <div className={`${neuro.card} p-4 text-center`}>
            <p className="text-3xl font-bold text-[#9aa3b8]">
              {resumen.accionesPorCategoria.APROBACION || 0}
            </p>
            <p className="text-xs text-[#69738c]">Aprobaciones</p>
          </div>
          <div className={`${neuro.card} p-4 text-center`}>
            <p className="text-3xl font-bold text-[#9aa3b8]">
              {resumen.accionesPorCategoria.DOCUMENTO || 0}
            </p>
            <p className="text-xs text-[#69738c]">Documentos</p>
          </div>
          <div className={`${neuro.card} p-4 text-center`}>
            <p className="text-lg font-bold text-[#9aa3b8]">
              {resumen.primeraAccion?.toLocaleDateString('es-CL', { day: '2-digit', month: 'short' })}
            </p>
            <p className="text-xs text-[#69738c]">Primera acción</p>
          </div>
        </div>
        ...
        <div className={`${neuro.panel} p-6`}>
          {Object.entries(historialPorFecha).map(([fecha, registros]) => (
            <div key={fecha} className="mb-8 last:mb-0">
              ...
              <div className="flex items-center gap-3 mb-4 ml-3">
                ...
                <span className="font-semibold text-[#69738c]">{fecha}</span>
          ...
          {historialFiltrado.length === 0 && (
            <div className="text-center py-12">
              <History className="w-16 h-16 text-[#9aa3b8] mx-auto mb-4" />
              <p className="text-[#69738c]">No hay registros que mostrar</p>
            </div>
          )}
        </div>
        ...
        <div className={`${neuro.card} p-4 mt-6 text-center`}>
          <div className="flex items-center justify-center gap-3 text-sm text-[#69738c]">
            <Lock className="w-4 h-4" />
            <span>Registro protegido • Hash cadena: <code className="text-[#6888ff]">{historial[historial.length - 1]?.hashRegistro.substring(0, 16)}...</code></span>
            <span>•</span>
