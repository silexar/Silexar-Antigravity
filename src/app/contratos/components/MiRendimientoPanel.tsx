  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-black text-slate-800">Mi Rendimiento</h2>
          <p className="text-sm text-slate-500">Período: Marzo 2025</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 text-white shadow-xl">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-indigo-200 text-xs font-bold uppercase tracking-wider">Meta Mensual</p>
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-white/20">
          ...
          <div>
            <p className="text-xs text-indigo-200">Restante</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard icon={<FileText className="w-5 h-5 text-indigo-500" />} label="Contratos" valor={String(RENDIMIENTO.contratosEsteMes)}
      ...
      <div className="neo-card rounded-2xl p-5">
        <h3 className="font-bold text-sm text-slate-700 mb-4 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-indigo-500" /> Actividad esta semana
        </h3>
        <div className="flex items-end gap-3 h-28">
          {RENDIMIENTO.contratosSemana.map(d => (
            <div key={d.dia} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full rounded-t-lg bg-gradient-to-t from-indigo-500 to-indigo-400 transition-all"
                style={{ height: `${d.valor > 0 ? Math.max((d.valor / maxSemana) * 100, 8) : 4}%` }} />
              <span className="text-[10px] font-bold text-slate-500">{d.dia}</span>
              {d.valor > 0 && <span className="text-[9px] text-slate-400">${(d.valor / 1000000).toFixed(0)}M</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
          ...
          <div className="flex items-end gap-2 mt-1">
            <p className="text-2xl font-black text-slate-800">{RENDIMIENTO.tasaCierre}%</p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-800 mt-1">${(RENDIMIENTO.ticketPromedio / 1000000).toFixed(1)}M</p>
        </div>
      </div>
    </div>
  );
