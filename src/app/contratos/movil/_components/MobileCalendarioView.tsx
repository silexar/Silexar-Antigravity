 ...
 * 
 * Calendario mensual compacto con event dots, panel de detalle por d�a,
 * y categorizaci�n de eventos. Adaptaci�n de calendario/page.tsx.
 * 
...
const DIAS_SEMANA = ['Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'S�', 'Do'];
...
const TIPO_CONFIG: Record<TipoEvento, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  renovacion: { label: 'Renovaci�n', color: 'text-[#6888ff]', bg: 'bg-[#6888ff]/50', icon: RefreshCw },
  reunion: { label: 'Reuni�n', color: 'text-[#6888ff]', bg: 'bg-[#6888ff]/50', icon: Users },
  pago: { label: 'Pago', color: 'text-[#6888ff]', bg: 'bg-[#6888ff]/50', icon: DollarSign },
  obligacion: { label: 'Obligaci�n', color: 'text-[#6888ff]', bg: 'bg-[#6888ff]/50', icon: FileText },
...
function generarEventosMock(): EventoCalendario[] {
  ...
  const acciones = ['Renovar contrato', 'Reuni�n ejecutiva', 'Fecha de pago', 'Entrega material', 'Revisi�n trimestral', 'Vencimientos p�liza', 'Firma digital'];
...
export function MobileCalendarioView() {
  ...
  return (
    <div className="space-y-5">
      {/* MONTH HEADER */}
      <div className="flex items-center justify-between bg-[#dfeaff] rounded-2xl p-3 shadow-sm border border-[#bec8de30]">
        <button onClick={() => cambiarMes(-1)} className="p-2 rounded-xl bg-[#dfeaff] active:bg-[#dfeaff]">
          <ChevronLeft className="w-5 h-5 text-[#69738c]" />
        </button>
        <div className="text-center">
          <p className="font-bold text-[#69738c]">{MESES[month]} {year}</p>
          <p className="text-[10px] text-[#9aa3b8]">{monthEvents.length} eventos</p>
        </div>
        <button onClick={() => cambiarMes(1)} className="p-2 rounded-xl bg-[#dfeaff] active:bg-[#dfeaff]">
          <ChevronRight className="w-5 h-5 text-[#69738c]" />
        </button>
      </div>
      ...
      <div className="bg-[#dfeaff] rounded-2xl p-4 shadow-sm border border-[#bec8de30]">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {DIAS_SEMANA.map(d => (
            <div key={d} className="text-center text-[10px] font-bold text-[#9aa3b8] uppercase py-1">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-xl flex flex-col items-center justify-center relative transition-all active:scale-90 ${
                  isSelected ? 'bg-[#6888ff] text-white shadow-lg shadow-[#6888ff]/20' :
                  isToday ? 'bg-[#dfeaff] text-[#6888ff]' :
                  'text-[#69738c]'
                }`}
              >
                    ...
                    {dayEvents.slice(0, 3).map((ev, j) => (
                      <div
                        ...
                        className={`w-1 h-1 rounded-full ${
                          isSelected ? 'bg-[#dfeaff]/70' : TIPO_CONFIG[ev.tipo]?.bg || 'bg-[#9aa3b8]'
                        }`}
                      />
                    ))}
                  </div>
      ...
      <div>
        <div className="flex items-center justify-between px-1 mb-3">
          <p className="text-xs font-bold text-[#9aa3b8] uppercase tracking-widest flex items-center gap-2">
            <Calendar className="w-3.5 h-3.5 text-[#6888ff]" />
            {new Date(selectedDate + 'T12:00:00').toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'short' })}
          </p>
          <span className="text-xs font-bold text-[#6888ff]">{selectedEventsFiltered.length} eventos</span>
        </div>
        ...
        {selectedEventsFiltered.length === 0 ? (
          <div className="bg-[#dfeaff] rounded-xl border border-[#bec8de30] p-8 text-center">
            ...
            <p className="text-sm font-bold text-[#9aa3b8]">Sin eventos para este d�a</p>
          </div>
        ) : (
      ...
      {eventos.filter(e => e.prioridad === 'alta' && !e.completado && e.fecha >= hoyStr).length > 0 && (
        <div>
          <p className="text-xs font-bold text-[#9aa3b8] uppercase tracking-widest mb-3 px-1 flex items-center gap-1">
            <AlertTriangle className="w-3.5 h-3.5" /> Pr�ximos Urgentes
          </p>
          <div className="space-y-2">
            {eventos
...
function EventCard({ evento, showDate }: { evento: EventoCalendario; showDate?: boolean }) {
  ...
  return (
    <div className={`bg-[#dfeaff] rounded-xl border border-[#bec8de30] p-4 flex items-start gap-3 ${
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-bold text-sm ${evento.completado ? 'text-[#9aa3b8] line-through' : 'text-[#69738c]'}`}>
          {evento.titulo}
        </p>
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          {evento.clienteNombre && (
            <span className="text-xs text-[#9aa3b8]">{evento.clienteNombre}</span>
          )}
          {evento.horaInicio && (
            <span className="text-xs text-[#9aa3b8] flex items-center gap-0.5">
              <Clock className="w-3 h-3" /> {evento.horaInicio}
              {evento.horaFin && ` - ${evento.horaFin}`}
            </span>
          )}
          {showDate && (
            <span className="text-xs text-[#6888ff] font-bold">
