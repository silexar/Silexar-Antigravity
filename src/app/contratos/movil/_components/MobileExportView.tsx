...
const TIPOS: { id: TipoReporte; label: string; desc: string }[] = [
  ...
  { id: 'facturacion', label: 'Facturaci�n', desc: 'Facturas emitidas y pagadas' },
  { id: 'analytics', label: 'Analytics', desc: 'M�tricas y tendencias' },
];
...
const FORMATOS: { id: FormatoExport; label: string; icon: React.ReactNode; desc: string }[] = [
  ...
  { id: 'excel', label: 'Excel', icon: <FileSpreadsheet className="w-5 h-5" />, desc: 'Hoja de c�lculo' },
];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-[#6888ff] rounded-2xl p-5 text-white shadow-xl">
        ...
        <p className="text-sm text-[#9aa3b8]">Genera reportes y datos del m�dulo contratos en el formato que necesites.</p>
      </div>

      {/* TIPO */}
      <div>
        <p className="text-xs font-bold text-[#9aa3b8] uppercase tracking-widest mb-3 px-1">�Qu� exportar?</p>
      ...
      {/* SUCCESS */}
      {completado && (
        <div className="bg-[#6888ff]/5 border border-[#bec8de30] rounded-xl p-4 flex items-center gap-3">
          <CheckCircle2 className="w-8 h-8 text-[#6888ff]" />
          <div className="flex-1">
            <p className="font-bold text-[#6888ff] text-sm">Archivo generado</p>
            <p className="text-xs text-[#6888ff]">reporte-{tipo}-{new Date().toISOString().split('T')[0]}.{formato}</p>
          </div>
          <button className="p-2 rounded-lg bg-[#6888ff]/10 active:scale-90"><Share2 className="w-4 h-4 text-[#6888ff]" /></button>
        </div>
      )}
    </div>
  );
