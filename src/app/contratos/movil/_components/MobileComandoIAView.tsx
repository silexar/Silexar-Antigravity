...
const SUGERENCIAS = [
  '�Cu�ntos contratos vencen este mes?',
  'Resume el estado del pipeline',
  '�Qu� clientes tienen pagos pendientes?',
  'Analiza tendencia de cierre Q1',
  'Top 5 contratos por valor',
];

const RESPUESTAS_MOCK: Record<string, string> = {
  'vencen': '?? Este mes vencen **12 contratos** por un valor total de **$185M**. Los m�s urgentes:\n\n1. **Falabella** � CTR-0045 � $45M (vence en 3 d�as)\n2. **Banco Chile** � CTR-0067 � $32M (vence en 5 d�as)\n3. **TechCorp** � CTR-0089 � $28M (vence en 7 d�as)\n\n?? Recomiendo contactar a Falabella hoy para renovaci�n.',
  'pipeline': '?? **Pipeline actual:**\n\n� Prospecci�n: 23 contratos ($450M)\n� Negociaci�n: 15 contratos ($320M)\n� Cierre: 8 contratos ($180M)\n� Firmado: 45 contratos ($890M)\n\n? Tasa de conversi�n: 68% (arriba del benchmark 62%)',
  'pendientes': '?? **3 clientes con pagos pendientes:**\n\n1. Ripley � $15.8M (17 d�as vencido)\n2. Falabella � $12.5M (12 d�as vencido)\n3. Banco Chile � $8.7M (en gesti�n)\n\nTotal pendiente: **$37M**',
  'default': '?? Entendido. Analizando los datos del m�dulo de contratos...\n\nBasado en la informaci�n disponible, puedo observar que el m�dulo cuenta con **156 contratos activos** por un valor total de **$2.4B**. La tasa de renovaci�n es del 78%, superior al promedio del mercado.\n\n�Te gustar�a que profundice en alg�n aspecto espec�fico?',
};

export function MobileComandoIAView() {
  const [mensajes, setMensajes] = useState<MensajeChat[]>([
    { id: 'welcome', rol: 'assistant', contenido: '?? �Hola! Soy tu asistente de contratos. Puedo analizar datos, generar reportes, y responder preguntas sobre el estado de tus contratos.\n\n�En qu� puedo ayudarte?', timestamp: new Date(), tipo: 'texto' },
  ]);
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 180px)' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#6888ff]/40 animate-pulse" />
              <span className="text-[10px] text-[#6888ff]">Online � Analizando 156 contratos</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2 pb-3">
          {SUGERENCIAS.map((s) => (
            <button key={s} onClick={() => enviar(s)}
              className="px-3 py-1.5 bg-[#6888ff]/5 text-[#6888ff] rounded-full text-[11px] font-bold border border-[#bec8de30] active:scale-95">
              {s}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-2 pt-2 border-t border-[#bec8de30]">
        ...
        <button onClick={() => enviar(input)} disabled={!input.trim() || pensando}
          className="px-4 py-3 bg-[#6888ff] text-white rounded-xl active:scale-90 disabled:opacity-50">
