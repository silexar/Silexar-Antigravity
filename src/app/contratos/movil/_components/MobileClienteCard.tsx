...
const CL = {
  ...
  contacto: 'Jos� Rodr�guez', tel: '+56 2 2345 6789', email: 'jrodriguez@bancochile.cl',
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-[#6888ff] rounded-2xl p-4 text-white shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-[#6888ff]">{CL.rubro} � {CL.contacto}</p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        <MiniStat icon={<DollarSign className="w-3 h-3 text-[#6888ff]" />} val={`$${(CL.valorHist / 1e6).toFixed(0)}M`} lbl="Hist�rico" />
        <MiniStat icon={<FileText className="w-3 h-3 text-[#6888ff]" />} val={String(CL.contratos)} lbl="Contratos" />
        <MiniStat icon={<TrendingUp className="w-3 h-3 text-[#6888ff]" />} val={`${CL.tasaRenov}%`} lbl="Renovaci�n" />
