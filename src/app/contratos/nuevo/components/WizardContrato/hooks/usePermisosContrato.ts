...
export function usePermisosContrato(usuario?: UsuarioActual): PermisosContrato {
  ...
  const puedeAprobarDescuento = useCallback((porcentaje: number): PermisoAprobacion => {
    ...
    return {
      ...
      mensaje: puedeAprobar 
        ? undefined 
        : `Descuento de ${porcentaje}% requiere aprobación de ${nivelRequerido.replace('_', ' ')}`
    };
  }, [tienePermiso]);
  ...
  const permisos = useMemo((): PermisosContrato => ({
    ...
    puedeEliminarEvidencia: tienePermiso(PERMISOS_ANTI_FRAUDE.ELIMINAR_EVIDENCIA),
    ...
    // Admin
    puedeOverrideAprobacion: tienePermiso(PERMISOS_ANTI_FRAUDE.OVERRIDE_APROBACION),
    puedeVerHistorialCompleto: tienePermiso(PERMISOS_ANTI_FRAUDE.VER_HISTORIAL_COMPLETO),
    
    // Utilidades
    nivelActual: usuarioActual.nivel,
    esAprobador: ['jefatura_directa', 'gerente_comercial', 'gerente_general', 'admin'].includes(usuarioActual.nivel),
    esAdmin: usuarioActual.nivel === 'admin'
  }), [tienePermiso, puedeAprobarDescuento, puedeCargarCampana, usuarioActual.nivel]);
...
export const getNivelRequeridoParaDescuento = (porcentaje: number): NivelJerarquico => {
