    ...
    >
      ...
      >
        {esRequerido
          ...
          : <div className="w-3 h-3 rounded-full bg-[#dfeaff]/50" />}
      </div>
      <div className="flex-1">
        <h4
          className={`font-medium ${
            esRequerido ? "text-[#69738c]" : "text-[#9aa3b8]"
          }`}
        >
          {titulo}
        </h4>
        <p
          className={`text-sm ${
            esRequerido ? "text-[#9aa3b8]" : "text-[#9aa3b8]"
          }`}
        >
          {descripcion}
        </p>
...
const FlujoAprobacionVisual: React.FC<{
  ...
  const niveles: {
  ...
  }[] = [
    {
      ...
      descripcion: "Sin intervención requerida",
    },
    {
      nivel: "supervisor",
      titulo: "Supervisor",
      descripcion: "Límite 2 horas",
    },
    {
      nivel: "gerente_comercial",
      titulo: "Gerente Comercial",
      descripcion: "Límite 4 horas",
    },
    {
      nivel: "gerente_general",
      titulo: "Gerente General",
      descripcion: "Límite 24 horas",
    },
    {
      nivel: "directorio",
      titulo: "Directorio",
      descripcion: "Límite 48 horas",
    },
  ];
      ...
      {aprobadores.aprobadores.map((aprobador, idx) => (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="font-medium text-[#69738c]">{aprobador.nombre}</h4>
              <p className="text-sm text-[#9aa3b8]">{aprobador.cargo}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="flex items-center gap-1 text-sm text-[#9aa3b8]">
                <Clock className="w-4 h-4" />
                <span>
                  {new Date(aprobador.tiempoLimite).toLocaleTimeString(
                    "es-CL",
                    {
                      hour: "2-digit",
                      minute: "2-digit",
              ...
              <p className="text-xs text-[#9aa3b8]">Tiempo límite</p>
            </div>
            <div
              className={`
            ...
            `}
            >
              {aprobador.estado.charAt(0).toUpperCase() +
                aprobador.estado.slice(1)}
            </div>
          </div>
        </motion.div>
      ))}
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#9aa3b8]">
            Configure el proceso de aprobación del contrato
          </p>
        </div>
      </div>

      {/* Resumen del contrato */}
      <div className="p-5 rounded-2xl bg-[#dfeaff] shadow-[inset_4px_4px_8px_#bec8de,inset_-4px_-4px_8px_#ffffff]">
        <div className="grid grid-cols-4 gap-6">
          <div>
            <p className="text-sm text-[#9aa3b8]">Valor del Contrato</p>
            <p className="text-xl font-bold text-[#6888ff]">
              {formatCurrency(state.valorNeto)}
            </p>
          </div>
          <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl bg-[#dfeaff] shadow-[8px_8px_16px_#bec8de,-8px_-8px_16px_#ffffff]">
          ...
          {/* Motivo */}
          <div className="mt-4 p-3 rounded-lg bg-[#6888ff15] border border-[#6888ff20]">
          ...
          <button
            ...
            className="mt-4 w-full py-2 text-sm text-[#6888ff] font-medium hover:bg-[#6888ff15] rounded-lg transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="p-6 rounded-2xl bg-[#dfeaff] shadow-[8px_8px_16px_#bec8de,-8px_-8px_16px_#ffffff]">
        <div className="flex items-center gap-3 mb-5">
          <Settings className="w-6 h-6 text-[#9aa3b8]" />
          <h3 className="text-lg font-semibold text-[#69738c]">
        ...
        <div className="space-y-4">
          <Toggle
            ...
            onChange={(checked) =>
              dispatch({ type: "SET_NOTIFICACIONES", payload: checked })}
            icon={Bell}
          />
          ...
          <Toggle
            ...
            onChange={(checked) =>
              dispatch({ type: "SET_ESCALAMIENTO_AUTO", payload: checked })}
            icon={Zap}
          />
        </div>
      </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700">
                  ...
                  {state.flujoAprobacion.aprobadores.length * 24}+ horas.
                  Considere revisar los términos si necesita una aprobación más
                  rápida.
                </p>
              </div>
            </div>
