  ...
  'cliente': [
    {
      ...
      mensaje: 'Al seleccionar un cliente existente, sus datos se autocompletarán. Si es un cliente nuevo, puedes crearlo directamente aquí.',
      paso: 'cliente',
      relevancia: 100
    },
    {
      ...
      mensaje: 'Asegúrate de tener el RUT correcto del cliente. El sistema validará automáticamente el dígito verificador.',
  ...
  'lineas': [
    {
      ...
      mensaje: 'Basado en contratos similares, este cliente típicamente contrata Radio FM + Digital. Considera agregar ambos medios.',
      accionable: true,
    ...
    },
    {
      ...
      mensaje: 'Puedes importar líneas directamente desde una cotización existente para ahorrar tiempo.',
      accionable: true,
    ...
    },
    {
      ...
      mensaje: 'Las tarifas mostradas están actualizadas al 1 de enero 2025. Consulta con tu supervisor para tarifas especiales.',
  ...
  'terminos': [
    {
      ...
      mensaje: 'Este cliente tiene score crediticio AAA. Puedes ofrecer hasta 60 días sin aumentar el riesgo.',
      paso: 'terminos',
      relevancia: 100
    },
    {
      ...
      mensaje: 'El descuento del 18% excede tu límite autorizado (15%). Necesitarás aprobación de nivel 2.',
      paso: 'terminos',
      relevancia: 100
    },
    {
      ...
      tipo: 'tip',
      titulo: '💡 Calculadora',
      mensaje: 'Usa la calculadora inteligente (botón 🧮) para simular diferentes escenarios de descuento y comisión.',
      accionable: true,
  ...
  'documentacion': [
    {
      ...
      mensaje: 'Puedes arrastrar correos directamente desde Gmail a esta área para adjuntar toda la negociación.',
      paso: 'documentacion',
      relevancia: 95
    },
    {
      ...
      mensaje: 'Cortex detectó una posible Orden de Compra en el email "Re: Propuesta comercial". ¿Quieres procesarla?',
      accionable: true,
  ...
  'aprobaciones': [
    {
      ...
      mensaje: 'Por el monto ($82M) y descuento (18%), se requieren 2 niveles de aprobación: Supervisor + Gerente Comercial.',
      paso: 'aprobaciones',
      relevancia: 100
    },
    {
      ...
      mensaje: 'Ana García (Supervisora) está activa ahora. El tiempo promedio de aprobación es de 2 horas.',
      paso: 'aprobaciones',
      relevancia: 85
    }
  ],
  'resumen': [
    {
      ...
      mensaje: 'Antes de enviar, ejecuta el checklist de verificación para asegurar que todo esté correcto.',
      accionable: true,
    ...
    },
    {
      ...
      mensaje: 'Genera un preview del PDF para revisar cómo quedará el contrato antes de enviarlo.',
      accionable: true,
...
const getTipoConfig = (tipo: TipoAyuda) => {
  const configs = {
    tip: { icon: <Lightbulb className="w-4 h-4" />, color: 'bg-blue-100 text-blue-600', border: 'border-blue-200' },
    sugerencia: { icon: <Sparkles className="w-4 h-4" />, color: 'bg-purple-100 text-purple-600', border: 'border-purple-200' },
    advertencia: { icon: <AlertTriangle className="w-4 h-4" />, color: 'bg-amber-100 text-amber-600', border: 'border-amber-300' },
    guia: { icon: <BookOpen className="w-4 h-4" />, color: 'bg-slate-100 text-slate-600', border: 'border-slate-200' },
    ia: { icon: <Sparkles className="w-4 h-4" />, color: 'bg-indigo-100 text-indigo-600', border: 'border-indigo-200' }
  };
  return configs[tipo];
};
...
export default function WizardAssistant({
  ...
  const ayudaSeleccionada = ayudas.find(a => a.id === ayudaActiva) || ayudas[0];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-indigo-600" />
          <span className="font-semibold text-sm text-slate-800">Asistente</span>
        </div>
        ...
        >
          ...
          <div className="mt-3 space-y-2">
            {ayudaSeleccionada.accionable && (
              <button
                onClick={() => onAccion?.(ayudaSeleccionada.id, ayudaSeleccionada.accionTexto || '')}
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
            <span className="text-xs text-slate-400">¿Te fue útil?</span>
            <div className="flex items-center gap-2">
              <button
      ...
      <div className="mt-3 flex items-center justify-center">
        <button className="text-xs text-slate-400 flex items-center gap-1 hover:text-indigo-600 transition-colors">
