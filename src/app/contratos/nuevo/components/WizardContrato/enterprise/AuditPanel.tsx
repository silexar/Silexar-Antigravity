        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span
                className={`px-2 py-0.5 rounded text-xs ${riesgoConfig.bg} ${riesgoConfig.color}`}
              >
                {evento.nivelRiesgo}
              </span>
            ...
            <p className="text-sm text-[#69738c] truncate">
              {evento.descripcion}
            </p>
          </div>
          ...
          <div className="flex items-center gap-6 text-sm text-[#9aa3b8]">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {evento.timestamp.toLocaleTimeString("es-CL", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </span>
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="border-t border-[#bec8de40] bg-[#dfeaff] overflow-hidden"
          >
            <div className="p-4 grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-[#9aa3b8] mb-1">Recurso</p>
                <p className="text-sm font-medium">
                  {evento.recursoTipo}: {evento.recursoId}
                </p>
              </div>
              <div>
                <p className="text-xs text-[#9aa3b8] mb-1">Email</p>
                <p className="text-sm">{evento.usuarioEmail}</p>
              </div>
              <div>
                <p className="text-xs text-[#9aa3b8] mb-1">IP Address</p>
                <p className="text-sm font-mono">{evento.ipAddress}</p>
              </div>
              <div>
                <p className="text-xs text-[#9aa3b8] mb-1">Sesión</p>
                <p className="text-sm font-mono">{evento.sesionId}</p>
              </div>

              {evento.camposModificados &&
                evento.camposModificados.length > 0 && (
                <div className="col-span-2">
                  <p className="text-xs text-[#9aa3b8] mb-1">
                    Campos Modificados
                  </p>
              ...
              <div className="col-span-4">
                ...
                <p className="text-xs font-mono text-[#69738c] bg-[#dfeaff] px-2 py-1 rounded">
                  SHA-256: {evento.hash}
                </p>
              </div>
            </div>
...
export const AuditPanel: React.FC<AuditPanelProps> = ({
  ...
  const [filtroRiesgo, setFiltroRiesgo] = useState<
    EventoAuditoria["nivelRiesgo"] | "todos"
  >("todos");
  ...
  const eventosFiltrados = useMemo(() => {
    return eventos.filter((e) => {
      ...
      if (filtroRiesgo !== "todos" && e.nivelRiesgo !== filtroRiesgo) {
        return false;
      }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <button className="px-4 py-2 rounded-xl bg-[#dfeaff]/10 text-white flex items-center gap-2 hover:bg-[#dfeaff]/20 transition-colors">
                <Download className="w-4 h-4" />
                Exportar
                <ChevronDown className="w-4 h-4" />
              </button>
              <div className="absolute right-0 mt-2 w-40 bg-[#dfeaff] rounded-xl shadow-[8px_8px_16px_#bec8de,-8px_-8px_16px_#ffffff] border border-[#bec8de40] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                <button
                  onClick={() => handleExportar("csv")}
                  className="w-full px-4 py-2 text-left text-sm text-[#69738c] hover:bg-[#dfeaff] rounded-t-xl"
                >
                  Exportar CSV
                </button>
                <button
                  onClick={() => handleExportar("json")}
                  className="w-full px-4 py-2 text-left text-sm text-[#69738c] hover:bg-[#dfeaff] rounded-b-xl"
                >
                  Exportar JSON
                </button>
              </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={filtroRiesgo}
              onChange={(e) =>
                setFiltroRiesgo(e.target.value as typeof filtroRiesgo)}
          ...
          {/* Resumen */}
          <div className="flex items-center gap-4 ml-auto text-sm text-[#9aa3b8]">
        ...
        <div className="space-y-3">
          ...
          {eventosFiltrados.length === 0 && (
            <div className="p-12 text-center">
              <Shield className="w-12 h-12 text-[#9aa3b8] mx-auto mb-3" />
        ...
        <div className="mt-6 p-4 rounded-xl bg-[#dfeaff] border border-[#bec8de40]">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-[#69738c]">
              ...
              <p className="text-[#9aa3b8]">
                Este registro cumple con SOX, GDPR, e ISO 27001. Cada evento
                está firmado digitalmente y encadenado con hash SHA-256.
              </p>
            </div>
          </div>
        </div>
