...
export interface BorradorContrato {
  ...
  estado: 'PENDIENTE_VALIDACION' | 'VALIDADO' | 'ENVIADO' | 'ACEPTADO' | 'RECHAZADO';
...
const patrones = {
  cliente: /(?:para|cliente|anunciante|empresa)[:\s]+([A-Za-zÀ-ÿ\s]+?)(?:\.|,|$)/i,
  producto: /(?:campaña|pauta|producto|spots?)[:\s]+([A-Za-zÀ-ÿ0-9\s]+?)(?:\.|,|$)/i,
  fechas: /(?:del?\s*)?(\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?)\s*(?:al?|hasta|-)?\s*(\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?)?/i,
  valor: /\$\s*([\d.,]+)(?:\s*(mil|k|millones?|m|MM))?/i,
  medios: /(radio|tv|televisión|digital|redes|facebook|instagram|youtube|via\s*pública|pantallas?)/gi,
...
class WhatsAppIntegrationServiceClass {
  ...
  async extraerDatos(mensaje: MensajeWhatsApp): Promise<DatosExtraidos> {
    ...
    const clienteExiste = clienteNombre ? await this.verificarCliente(clienteNombre, mensaje.from) : false;
    ...
    if (matchValor) {
      ...
      if (multiplicador === 'millones' || multiplicador === 'm' || multiplicador === 'mm') num *= 1000000;
      valorMencionado = num;
    } else {
    ...
    const medios = matchMedios ? [...new Set(matchMedios.map(m => m.toLowerCase()))] : [];
    if (medios.length === 0) {
    ...
    const urgencia: 'alta' | 'normal' | 'baja' = matchUrgencia ? 'alta' : 'normal';
    if (urgencia === 'alta') {
    ...
    return {
      ...
      fechas: (fechaInicio || fechaFin) ? { inicio: fechaInicio, fin: fechaFin } : undefined,
      valores: valorMencionado ? { mencionado: valorMencionado, moneda: 'CLP' } : undefined,
      medios,
      urgencia,
      camposFaltantes,
      inconsistencias,
  ...
  private async verificarCliente(_nombre: string, _telefono: string): Promise<boolean> {
  ...
  private async crearBorrador(mensaje: MensajeWhatsApp, datos: DatosExtraidos): Promise<BorradorContrato> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private async notificarEjecutivo(_ejecutivoId: string, _borrador: BorradorContrato): Promise<void> {
  ...
  private generarRespuesta(datos: DatosExtraidos): RespuestaAutomatica {
    ...
    if (datos.confianza >= 70) {
      ...
      if (datos.producto) resumen.push(`• Campaña: ${datos.producto.descripcion}`);
      if (datos.fechas) resumen.push(`• Período: ${datos.fechas.inicio || '?'} - ${datos.fechas.fin || '?'}`);
      if (datos.valores) resumen.push(`• Valor: $${datos.valores.mencionado?.toLocaleString()}`);
      if (datos.medios?.length) resumen.push(`• Medios: ${datos.medios.join(', ')}`);

      return {
        tipo: 'confirmacion',
        mensaje: `¡Recibido! ✅\n\nHemos capturado la siguiente información:\n\n${resumen.join('\n')}\n\nUn ejecutivo revisará los detalles y le enviará una propuesta formal en breve.${datos.urgencia === 'alta' ? '\n\n⚡ Hemos marcado esto como URGENTE.' : ''}`,
        botonesRapidos: [
   ...
   */
  async validarBorrador(borradorId: string, ajustes: Partial<DatosExtraidos>): Promise<BorradorContrato> {
  ...
  getBorradoresPendientes(ejecutivoId: string): BorradorContrato[] {
    return Array.from(this.borradores.values())
      .filter(b => b.ejecutivoAsignado === ejecutivoId && b.estado === 'PENDIENTE_VALIDACION')
  ...
  getEstadisticas(): {
    ...
    return {
      ...
      pendientesValidacion: borradores.filter(b => b.estado === 'PENDIENTE_VALIDACION').length,
...
export const WhatsAppIntegrationService = WhatsAppIntegrationServiceClass.getInstance();
