...
const PATRONES_INTENCION: { patron: RegExp; intento: IntentoCopilot }[] = [
  ...
  { patron: /modificar|cambiar|editar.*contrato/i, intento: 'modificar_contrato' },
  ...
  { patron: /\?|cómo|qué|cuándo|dónde|por qué|cuál/i, intento: 'responder_pregunta' }
];
...
export class CopilotService {
  ...
  private extraerEntidades(mensaje: string): MensajeCopilot['entidades'] {
    ...
    // Detectar montos
    const montoMatch = mensaje.match(/\$?\s*(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)\s*(millones?|M)?/i);
    if (montoMatch) {
      let monto = parseFloat(montoMatch[1].replace(/\./g, '').replace(',', '.'));
      if (montoMatch[2]) monto *= 1000000;
      entidades.push({ tipo: 'monto', valor: monto.toString(), confianza: 0.9 });
    }
    ...
    if (/hoy|mañana|próxima semana|próximo mes|enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre/i.test(mensaje)) {
      entidades.push({ tipo: 'fecha', valor: 'detectada', confianza: 0.7 });
    }
    
    // Detectar tipos de medio
    const medios = ['radio', 'tv', 'televisión', 'digital', 'prensa', 'exterior'];
  ...
  private async generarRespuesta(
    ...
    switch (intento) {
        ...
        if (estadoWizard?.anunciante) {
          ...
          return {
            texto: this.getRespuesta('alerta_riesgo')
              ...
              .replace('{nivel}', nivel)
              .replace('{dias}', estadoWizard.anunciante.terminosPreferenciales.diasPago.toString()),
            acciones: [{
              tipo: 'modificar',
              titulo: 'Ajustar Términos de Pago',
              descripcion: `Cambiar a ${estadoWizard.anunciante.terminosPreferenciales.diasPago} días`,
              requiereConfirmacion: true
            }]
          };
        }
        return {
          texto: 'Para analizar el riesgo, primero necesito que selecciones un anunciante.'
        };
      ...
      case 'generar_clausula':
        return {
          texto: '¿Qué tipo de cláusula necesitas? Puedo sugerirte:\n\n• Cláusula de exclusividad\n• Cláusula de confidencialidad\n• Cláusula de penalización\n• Cláusula de renovación automática',
          acciones: [
            { tipo: 'generar', titulo: 'Exclusividad', descripcion: 'Agregar cláusula de exclusividad', requiereConfirmacion: true },
            { tipo: 'generar', titulo: 'Confidencialidad', descripcion: 'Agregar cláusula NDA', requiereConfirmacion: true }
          ]
        };
   ...
   */
  getSugerenciasContextuales(estadoWizard: WizardContratoState): SugerenciaCopilot[] {
    ...
    switch (estadoWizard.currentStep) {
      case 1:
        ...
        if (!estadoWizard.anunciante && estadoWizard.tipoContrato === 'nuevo') {
          sugerencias.push({
            ...
            razonamiento: 'Los clientes frecuentes tienen mejores condiciones preferenciales',
            confianza: 0.8,
      ...
      case 2:
        ...
        if (estadoWizard.anunciante) {
          const descuentoMax = estadoWizard.anunciante.terminosPreferenciales.descuentoMaximo;
          if (estadoWizard.descuentoPorcentaje === 0) {
            sugerencias.push({
              tipo: 'descuento',
              valor: Math.floor(descuentoMax * 0.7),
              razonamiento: `Cliente con score ${estadoWizard.anunciante.scoreRiesgo}. Descuento óptimo para mantener rentabilidad.`,
              confianza: 0.85,
        ...
        if (estadoWizard.anunciante && estadoWizard.terminosPago.diasPago > estadoWizard.anunciante.terminosPreferenciales.diasPago) {
          sugerencias.push({
            tipo: 'termino',
            valor: { diasPago: estadoWizard.anunciante.terminosPreferenciales.diasPago },
            razonamiento: `El cliente tiene términos preferenciales de ${estadoWizard.anunciante.terminosPreferenciales.diasPago} días`,
            confianza: 0.9,
  ...
  private async sugerirTerminos(estadoWizard?: WizardContratoState): Promise<{
    ...
    if (!estadoWizard?.anunciante) {
      return { texto: 'Primero selecciona un anunciante para que pueda sugerirte términos personalizados.' };
    }
    
    const { terminosPreferenciales, scoreRiesgo, nivelRiesgo } = estadoWizard.anunciante;
    
    let texto = `📊 **Análisis de Términos para ${estadoWizard.anunciante.nombre}**\n\n`;
    texto += `• **Score de Riesgo:** ${scoreRiesgo}/1000 (${nivelRiesgo})\n`;
    texto += `• **Días de Pago Sugeridos:** ${terminosPreferenciales.diasPago} días\n`;
    texto += `• **Descuento Máximo:** ${terminosPreferenciales.descuentoMaximo}%\n`;
    texto += `• **Límite de Crédito:** $${terminosPreferenciales.limiteCredito.toLocaleString()}\n\n`;
    
    if (nivelRiesgo === 'bajo') {
      texto += '✅ Este cliente tiene excelente historial. Puedes ofrecer condiciones flexibles.';
    } else if (nivelRiesgo === 'medio') {
      texto += '⚠️ Cliente con riesgo moderado. Recomiendo términos estándar.';
    } else {
      texto += '🚨 Cliente de alto riesgo. Sugiero requerir garantía o pago anticipado.';
    }
    ...
    return {
      ...
      acciones: [{
        ...
        descripcion: `Configurar ${terminosPreferenciales.diasPago} días de pago`,
        requiereConfirmacion: true
      }]
    };
  }
  ...
  private generarResumenContrato(estado: WizardContratoState): string {
    ...
    if (estado.fechaInicio && estado.fechaFin) {
      resumen += `**Vigencia:** ${estado.fechaInicio.toLocaleDateString()} - ${estado.fechaFin.toLocaleDateString()}\n`;
    }
    ...
    if (estado.flujoAprobacion) {
      ...
      resumen += `**Aprobadores:** ${estado.flujoAprobacion.aprobadores.length}\n`;
    }
  ...
  private responderPregunta(
    ...
    if (preguntaLower.includes('cómo') && preguntaLower.includes('contrato')) {
      return {
        texto: 'Para crear un contrato, sigue estos pasos:\n\n1. Selecciona el tipo de contrato y cliente\n2. Configura los términos comerciales\n3. Agrega las líneas de especificación\n4. Revisa el flujo de aprobación\n5. Genera y firma los documentos\n\n¿En qué paso necesitas ayuda específica?'
      };
    }
    
    // Preguntas sobre descuentos
    if (preguntaLower.includes('descuento') && preguntaLower.includes('máximo')) {
      if (estadoWizard?.anunciante) {
        return {
          texto: `El descuento máximo para ${estadoWizard.anunciante.nombre} es **${estadoWizard.anunciante.terminosPreferenciales.descuentoMaximo}%** según su score de riesgo.`
        };
      }
      return { texto: 'Selecciona un cliente primero para ver el descuento máximo permitido.' };
    }
    
    // Preguntas sobre aprobación
    if (preguntaLower.includes('aprobación') || preguntaLower.includes('aprobar')) {
      return {
        texto: 'El nivel de aprobación se determina automáticamente según:\n\n• Valor del contrato\n• Porcentaje de descuento\n• Días de pago\n• Si es cliente nuevo o recurrente\n\nContratos menores a $10M con descuento ≤10% y cliente recurrente se aprueban automáticamente.'
      };
    }
    
    return {
      texto: 'No estoy seguro de entender tu pregunta. ¿Podrías reformularla o ser más específico? Puedo ayudarte con:\n\n• Crear o modificar contratos\n• Analizar riesgo de clientes\n• Sugerir términos comerciales\n• Explicar el proceso de aprobación'
    };
  }
...
export function useCopilot(estadoWizard?: WizardContratoState) {
  ...
  return {
    procesarMensaje: (mensaje: string) => copilot.procesarMensaje(mensaje, estadoWizard),
    getSugerencias: () => estadoWizard ? copilot.getSugerenciasContextuales(estadoWizard) : [],
