...
class ImmutableAuditEngine {
    ...
    if (filtros) {
      ...
      if (filtros.tipoAccion) {
        historial = historial.filter((r) =>
          r.tipoAccion === filtros.tipoAccion
        );
      }
  ...
  getResumen(contratoId: string): ResumenHistorial {
    ...
    historial.forEach((r) => {
      accionesPorCategoria[r.categoria] =
        (accionesPorCategoria[r.categoria] || 0) + 1;
    });
    ...
    return {
      ...
      usuariosUnicos,
      accionesPorCategoria: accionesPorCategoria as Record<
        CategoriaAccion,
        number
      >,
  ...
  verificarIntegridad(contratoId: string): boolean {
    ...
    for (let i = 0; i < historial.length; i++) {
      ...
      if (i === 0) {
        ...
        if (registro.hashAnterior !== historial[i - 1].hashRegistro) {
          return false;
        }
      }
  ...
  private inicializarDemoData(): void {
    ...
    const acciones = [
      ...
      },
      {
        ...
        categoria: "APROBACION" as CategoriaAccion,
        desc:
          "Aprobación nivel 1 otorgada - Descuento autorizado por cliente estratégico",
        user: {
          id: "u-002",
          nombre: "Ana García",
          email: "ana@silexar.cl",
    ...
    acciones.forEach((accion) => {
      this.registrar({
        ...
        usuario: accion.user,
        contexto: (accion as { contextoOverride?: typeof contextoBase })
          .contextoOverride || contextoBase,
