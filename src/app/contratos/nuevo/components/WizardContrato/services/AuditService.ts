...
export class AuditService {
  ...
  async logEvent(params: {
    ...
    const nivelRiesgo = this.calculateRiskLevel(params.tipoEvento, params.resultado);
  ...
  getContractAuditSummary(contratoId: string): {
    ...
    const ultimaModificacion = eventos.find(e => 
      ['editar', 'crear', 'aprobar', 'firmar', 'cambiar_estado'].includes(e.tipoEvento)
    );
   ...
   */
  exportForCompliance(formato: 'json' | 'csv', filtros?: Parameters<typeof this.getEvents>[0]): string {
    ...
    // CSV
    const headers = [
      'ID', 'Timestamp', 'Usuario', 'Email', 'Rol', 'Tipo Evento', 
      'Recurso', 'Accion', 'Descripcion', 'IP', 'Resultado', 'Nivel Riesgo', 'Hash'
    ];
  ...
  private getCurrentUser(): { id: string; nombre: string; email: string; rol: RolUsuario } {
    ...
    return (async (...args: unknown[]) => {
      ...
      try {
        ...
        await audit.logEvent({
          ...
          descripcion,
          resultado: 'fallo',
          motivoFallo: error instanceof Error ? error.message : 'Error desconocido'
        });
