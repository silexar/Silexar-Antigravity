...
export class FirmaDigitalService {
  ...
  private cargarDatos(): void {
    try {
      ...
      if (data) {
        ...
        this.eventos = (parsed.eventos || []).map((
          e: EventoFirma & { timestamp: string },
        ) => ({
          ...e,
  ...
  async crearSolicitud(solicitud: SolicitudFirma): Promise<EnvelopeFirma> {
    ...
    const envelope: EnvelopeFirma = {
      ...
      fechaExpiracion: new Date(
        Date.now() + (solicitud.expiracionDias || 30) * 24 * 60 * 60 * 1000,
      ),
    };
  ...
  async enviarParaFirma(envelopeId: string): Promise<EnvelopeFirma> {
    const envelope = Array.from(this.envelopes.values()).find((e) =>
  ...
  async simularFirma(
    ...
    const todosFirmaron = envelope.firmantes.every((f) =>
      f.estado === "firmado"
    );
    if (todosFirmaron) {
      envelope.estado = "firmado";
      envelope.fechaCompletado = new Date();
      envelope.urlDocumentoFirmado =
        `/documentos/firmados/${envelope.envelopeId}.pdf`;
    } else {
  ...
  async obtenerEstado(envelopeId: string): Promise<EnvelopeFirma | null> {
    return Array.from(this.envelopes.values()).find((e) =>
      e.envelopeId === envelopeId
    ) || null;
  }
  ...
  async cancelar(envelopeId: string, motivo: string): Promise<void> {
    ...
    this.guardarDatos();
    logger.info(
      `[FirmaDigital] Envelope cancelado: ${envelopeId}, motivo: ${motivo}`,
    );
  }

  async reenviarRecordatorio(
    envelopeId: string,
    firmanteEmail: string,
  ): Promise<void> {
    const envelope = Array.from(this.envelopes.values()).find((e) =>
    ...
    this.eventos.push({
      ...
      envelopeId,
      firmanteEmail,
      firmanteNombre: envelope.firmantes.find((f) =>
        f.email === firmanteEmail
      )?.nombre || "",
      timestamp: new Date(),
    });
  ...
  obtenerEstadisticas(): {
    ...
    return {
      ...
      pendientes:
        envelopes.filter((e) =>
          ["creado", "enviado", "en_proceso"].includes(e.estado)
        ).length,
      firmados: firmados.length,
      rechazados: envelopes.filter((e) => e.estado === "rechazado").length,
      tiempoPromedioFirma: firmados.length > 0
        ? tiempoTotal / firmados.length / (1000 * 60 * 60)
        : 0, // horas
    };
  }
}
...
export function useFirmaDigital(
  config?: Partial<FirmaDigitalConfig>,
): FirmaDigitalService {
