...
class EmailIngestConfig {
  ...
  private detectarTipoDocumento(nombre: string): TipoDocumento {
    ...
    if (nombreLower.includes("cotizacion") || nombreLower.includes("quote")) {
      return "COTIZACION";
    }
    if (nombreLower.includes("oc") || nombreLower.includes("orden")) {
      return "ORDEN_COMPRA";
    }
    ...
    if (nombreLower.includes("acta") || nombreLower.includes("minuta")) {
      return "ACTA_REUNION";
    }
    return "ADJUNTO";
  }
...
class DocumentationIngestionEngine {
  ...
  async procesarDragDrop(
    ...
    for (const file of files) {
      const doc = await this.procesarArchivo(
        file,
        contratoId,
        usuario,
        "DRAG_DROP",
  ...
  private detectarTipo(nombre: string, mime: string): TipoDocumento {
    ...
    if (mime.includes("message") || nombreLower.endsWith(".eml")) {
      return "EMAIL";
    }
    if (nombreLower.includes("cotizacion") || nombreLower.includes("quote")) {
      return "COTIZACION";
  ...
  private async procesarConIA(
    documentos: DocumentoNegociacion[],
  ): Promise<void> {
  ...
  private extraerDatosDeTexto(texto: string): DatosExtraidosDocumento {
    ...
    if (valorMatch) {
      let valor = parseFloat(
        valorMatch[0].replace(/[^\d.,]/g, "").replace(",", "."),
      );
      if (
        valorMatch[0].toLowerCase().includes("millon") ||
        valorMatch[0].includes("MM")
      ) {
        valor *= 1000000;
      }
    ...
    const compromisos = texto.match(
      /(?:acordamos|comprometemos|confirmo|queda acordado)[^.]+\./gi,
    );
    if (compromisos) {
  ...
  getResumenDocumentacion(contratoId: string): {
    ...
    docs.forEach((doc) => {
      ...
      if (doc.datosExtraidos) {
        if (
          ...
          !datosConsolidados.valorMencionado
        ) {
          datosConsolidados.valorMencionado =
            doc.datosExtraidos.valorMencionado;
        }
        if (doc.datosExtraidos.mediosMencionados) {
          datosConsolidados.mediosMencionados?.push(
            ...doc.datosExtraidos.mediosMencionados,
          );
        }
        if (doc.datosExtraidos.compromisos) {
          datosConsolidados.compromisos?.push(
            ...doc.datosExtraidos.compromisos,
          );
        }
      }
    });
    ...
    if (datosConsolidados.mediosMencionados) {
      datosConsolidados.mediosMencionados = [
        ...new Set(datosConsolidados.mediosMencionados),
      ];
    }
    ...
    return {
      ...
      confianzaPromedio: countConfianza > 0
        ? totalConfianza / countConfianza
        : 0,
    };
  }
...
export const DocumentationIngestion = DocumentationIngestionEngine
  .getInstance();
