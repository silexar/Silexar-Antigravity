...
export type EstadoAlerta = 'activa' | 'leida' | 'resuelta' | 'ignorada' | 'escalada';
...
class SmartAlertsEngine {
    ...
    this.alertas = [
      {
        ...
        mensaje: 'El contrato CTR-2025-001 vence en 15 días. Considere iniciar proceso de renovación.',
        ...
        creadoPor: 'SISTEMA',
        etiquetas: ['vencimiento', 'renovación']
      },
      {
        id: 'alerta-002',
        tipo: 'APROBACION_REQUERIDA',
        ...
        mensaje: 'El contrato CTR-2025-002 lleva 36 horas esperando aprobación de nivel 2.',
      ...
      },
      {
        ...
        mensaje: 'Se solicitó un descuento del 25% que excede el límite de 20% para el rol ejecutivo.',
      ...
      },
      {
        ...
        mensaje: 'El cliente ha aumentado su inversión un 40% este año. Considere ofrecer paquete premium.',
      ...
      },
      {
        ...
        mensaje: 'El contrato fue enviado hace 5 días y aún no ha sido firmado por el cliente.',
  ...
  async verificarYGenerarAlertas(): Promise<AlertaContrato[]> {
    ...
    logger.info(`Verificación de alertas ejecutada: ${new Date().toISOString()}`);
   ...
   * Crea alerta manual
   */
  crearAlerta(datos: Omit<AlertaContrato, 'id' | 'fechaCreacion' | 'estado'>): AlertaContrato {
   ...
   */
  actualizarConfiguracion(tipo: TipoAlerta, config: Partial<ConfiguracionAlerta>): void {
...
export function getAlertaInfo(tipo: TipoAlerta): { emoji: string; label: string; color: string } {
  const info: Record<TipoAlerta, { emoji: string; label: string; color: string }> = {
    VENCIMIENTO_PROXIMO: { emoji: '📅', label: 'Vencimiento próximo', color: 'bg-amber-100 text-amber-700' },
    RENOVACION_PENDIENTE: { emoji: '🔄', label: 'Renovación pendiente', color: 'bg-blue-100 text-blue-700' },
    APROBACION_REQUERIDA: { emoji: '✋', label: 'Aprobación requerida', color: 'bg-purple-100 text-purple-700' },
    DESCUENTO_EXCESIVO: { emoji: '💰', label: 'Descuento excesivo', color: 'bg-red-100 text-red-700' },
    PAGO_VENCIDO: { emoji: '💳', label: 'Pago vencido', color: 'bg-red-100 text-red-700' },
    DOCUMENTO_FALTANTE: { emoji: '📎', label: 'Documento faltante', color: 'bg-orange-100 text-orange-700' },
    FIRMA_PENDIENTE: { emoji: '✍️', label: 'Firma pendiente', color: 'bg-indigo-100 text-indigo-700' },
    ANOMALIA_DETECTADA: { emoji: '⚠️', label: 'Anomalía detectada', color: 'bg-red-100 text-red-700' },
    OPORTUNIDAD: { emoji: '🌟', label: 'Oportunidad', color: 'bg-green-100 text-green-700' },
    CUMPLIMIENTO: { emoji: '✅', label: 'Cumplimiento', color: 'bg-emerald-100 text-emerald-700' },
    SLA_RIESGO: { emoji: '⏱️', label: 'SLA en riesgo', color: 'bg-orange-100 text-orange-700' },
    INACTIVIDAD: { emoji: '💤', label: 'Inactividad', color: 'bg-slate-100 text-slate-700' }
  };
  return info[tipo];
}
