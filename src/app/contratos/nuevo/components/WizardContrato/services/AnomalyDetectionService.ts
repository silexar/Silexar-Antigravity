...
const mockAnomalias: Anomalia[] = [
  {
    ...
    descripcion: 'El contrato CTR-2025-0147 de Retail XYZ tiene un descuento de 35%, que excede el límite autorizado de 25% para el ejecutivo.',
    entidadAfectada: {
      tipo: 'contrato',
      id: 'ctr-147',
  ...
  },
  {
    ...
    descripcion: 'Empresa ABC tiene 3 facturas vencidas y score de riesgo de crédito de 82. Probabilidad de impago: 67%.',
    entidadAfectada: {
      tipo: 'cliente',
      id: 'cli-abc',
      nombre: 'Empresa ABC'
  ...
  },
  {
    ...
    descripcion: 'Banco Delta incrementó 45% inversión en competencia pero nuestro contrato lleva 6 meses sin crecimiento.',
    entidadAfectada: {
      tipo: 'cliente',
      id: 'cli-delta',
      nombre: 'Banco Delta'
  ...
  },
  {
    ...
    descripcion: 'El ejecutivo Juan Pérez tiene una tasa de cierre de 42% vs 68% promedio del equipo. Tendencia negativa por 3 meses.',
    entidadAfectada: {
      tipo: 'ejecutivo',
      id: 'ej-juanp',
      nombre: 'Juan Pérez'
  ...
  },
  {
    ...
    descripcion: 'Múltiples contratos con valores redondeados justo bajo umbral de aprobación ($24.9M). Revisión requerida.',
    entidadAfectada: {
      tipo: 'cartera',
      id: 'cart-001',
...
class AnomalyDetectionEngine {
   ...
   */
  async getAnomaliasBySeveridad(severidades: SeveridadAnomalia[]): Promise<Anomalia[]> {
  ...
  async analizarContrato(contrato: {
    ...
    if (contrato.descuento > 25) {
      anomalias.push({
        ...
        descripcion: `El descuento de ${contrato.descuento}% excede el límite estándar de 25%.`,
        entidadAfectada: {
          tipo: 'contrato',
          id: 'nuevo',
    ...
    const desviacion = Math.abs(contrato.valor - valorPromedio) / valorPromedio * 100;
    ...
    if (desviacion > 50) {
      anomalias.push({
        ...
        descripcion: `El valor es ${desviacion.toFixed(0)}% diferente al promedio histórico.`,
        entidadAfectada: {
          tipo: 'contrato',
          id: 'nuevo',
          nombre: 'Contrato en Creación'
    ...
    if (contrato.diasPago > 60) {
      anomalias.push({
        ...
        descripcion: `${contrato.diasPago} días de pago incrementa riesgo de cartera.`,
        entidadAfectada: {
          tipo: 'contrato',
          id: 'nuevo',
          nombre: 'Contrato en Creación'
        ...
        confianza: 90,
        recomendaciones: ['Considerar descuento por pronto pago', 'Evaluar historial de pagos del cliente'],
        acciones: [
   ...
   */
  async atenderAnomalia(id: string, accion: string, comentario?: string): Promise<void> {
    logger.info(`Atendiendo anomalía ${id} con acción ${accion}${comentario ? ': ' + comentario : ''}`);
