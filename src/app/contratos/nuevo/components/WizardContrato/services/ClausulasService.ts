...
const CLAUSULAS_BASE: Omit<
  ClausulaLegal,
  "id" | "fechaCreacion" | "ultimaModificacion" | "usosCount"
>[] = [
  {
    codigo: "CL-GEN-001",
    ...
    categoria: "general",
    contenido:
      'El presente contrato tiene por objeto regular la prestación de servicios publicitarios por parte de SILEXAR MEDIA GROUP SpA (en adelante "LA EMPRESA") al cliente {{CLIENTE_NOMBRE}} (en adelante "EL CLIENTE"), consistentes en la difusión de mensajes publicitarios a través de los medios y espacios que se detallan en el Anexo de Especificaciones.',
    contenidoHTML:
    ...
    variables: [
      {
        ...
        tipo: "texto",
        requerida: true,
        fuenteAutomatica: "anunciante.nombre",
      },
    ],
    version: 1,
    estado: "aprobada",
  ...
  },
  {
    ...
    categoria: "pago",
    contenido:
      "EL CLIENTE se compromete a pagar el valor total del contrato de {{VALOR_NETO}} {{MONEDA}}, en un plazo máximo de {{DIAS_PAGO}} días desde la emisión de la factura correspondiente. El pago se realizará mediante {{FORMA_PAGO}}.",
    contenidoHTML:
      "<p>EL CLIENTE se compromete a pagar el valor total del contrato de <strong>{{VALOR_NETO}} {{MONEDA}}</strong>, en un plazo máximo de <strong>{{DIAS_PAGO}} días</strong> desde la emisión de la factura correspondiente. El pago se realizará mediante <strong>{{FORMA_PAGO}}</strong>.</p>",
    variables: [
      {
        nombre: "VALOR_NETO",
        tipo: "moneda",
        requerida: true,
        fuenteAutomatica: "valorNeto",
      },
      {
        nombre: "MONEDA",
        tipo: "select",
        requerida: true,
        opciones: [{ valor: "CLP", etiqueta: "Pesos Chilenos" }, {
          valor: "USD",
          etiqueta: "Dólares",
        }, { valor: "UF", etiqueta: "UF" }],
        fuenteAutomatica: "moneda",
      },
      {
        nombre: "DIAS_PAGO",
        tipo: "numero",
        requerida: true,
        fuenteAutomatica: "terminosPago.diasPago",
      },
      {
        nombre: "FORMA_PAGO",
        tipo: "select",
  ...
  },
  {
    ...
    categoria: "pago",
    contenido:
      "En caso de mora en el pago, EL CLIENTE deberá pagar un interés del {{INTERES_MORA}}% mensual sobre el monto adeudado, sin perjuicio del derecho de LA EMPRESA de suspender los servicios contratados hasta que se regularice el pago.",
    contenidoHTML:
      "<p>En caso de mora en el pago, EL CLIENTE deberá pagar un interés del <strong>{{INTERES_MORA}}%</strong> mensual sobre el monto adeudado, sin perjuicio del derecho de LA EMPRESA de suspender los servicios contratados hasta que se regularice el pago.</p>",
    variables: [
      {
        nombre: "INTERES_MORA",
        tipo: "porcentaje",
  ...
  },
  {
    ...
    contenido:
      "Durante la vigencia del presente contrato, LA EMPRESA se compromete a no difundir publicidad de competidores directos de EL CLIENTE en la categoría de {{CATEGORIA_EXCLUSIVA}} dentro de los espacios y horarios contratados.",
    contenidoHTML:
      "<p>Durante la vigencia del presente contrato, LA EMPRESA se compromete a no difundir publicidad de competidores directos de EL CLIENTE en la categoría de <strong>{{CATEGORIA_EXCLUSIVA}}</strong> dentro de los espacios y horarios contratados.</p>",
    variables: [
      {
        nombre: "CATEGORIA_EXCLUSIVA",
        tipo: "texto",
        requerida: true,
        descripcion: "Categoría de producto o servicio con exclusividad",
      },
    ],
    version: 1,
    estado: "aprobada",
  ...
  },
  {
    ...
    contenido:
      "Cualquiera de las partes podrá dar término anticipado al presente contrato, notificando por escrito a la otra parte con al menos {{DIAS_PREAVISO}} días de anticipación. En caso de terminación anticipada por parte de EL CLIENTE, deberá pagar una indemnización equivalente al {{PORCENTAJE_INDEMNIZACION}}% del valor no ejecutado del contrato.",
    contenidoHTML:
    ...
    variables: [
      {
        nombre: "DIAS_PREAVISO",
        tipo: "numero",
        requerida: true,
        valorPorDefecto: "30",
      },
      {
        nombre: "PORCENTAJE_INDEMNIZACION",
        tipo: "porcentaje",
        requerida: true,
        valorPorDefecto: "50",
      },
  ...
  },
  {
    ...
    contenido:
      "Para todos los efectos legales derivados del presente contrato, las partes fijan su domicilio en la ciudad de {{CIUDAD_JURISDICCION}}, Chile, y se someten a la jurisdicción de sus tribunales ordinarios de justicia.",
    contenidoHTML:
      "<p>Para todos los efectos legales derivados del presente contrato, las partes fijan su domicilio en la ciudad de <strong>{{CIUDAD_JURISDICCION}}, Chile</strong>, y se someten a la jurisdicción de sus tribunales ordinarios de justicia.</p>",
    variables: [
      {
        nombre: "CIUDAD_JURISDICCION",
        tipo: "select",
        requerida: true,
        opciones: [{ valor: "Santiago", etiqueta: "Santiago" }, {
          valor: "Valparaíso",
          etiqueta: "Valparaíso",
        }, { valor: "Concepción", etiqueta: "Concepción" }],
        valorPorDefecto: "Santiago",
      },
    ],
    version: 1,
    estado: "aprobada",
...
export class ClausulasService {
  ...
  obtenerObligatorias(tipoContrato: string): ClausulaLegal[] {
    return this.obtenerParaTipoContrato(tipoContrato).filter((c) =>
      c.esObligatoria
    );
  }
  ...
  obtenerCategorias(): { categoria: CategoriaClausula; count: number }[] {
    const categorias: Record<CategoriaClausula, number> = {} as Record<
      CategoriaClausula,
      number
    >;
  ...
  sugerirClausulas(contexto: {
    ...
    const clausulasAplicables = this.obtenerParaTipoContrato(
      ...
      try {
        ...
        if (condicionCumplida) {
          ...
          if (clausula.categoria === "exclusividad") {
            razon =
              `Contrato de alto valor ($${contexto.valorNeto.toLocaleString()}). Recomendado para proteger inversión del cliente.`;
            prioridad = 8;
          } else if (
            clausula.categoria === "pago" && clausula.codigo.includes("MORA")
          ) {
            razon =
              `Cliente con nivel de riesgo ${contexto.nivelRiesgo}. Cláusula de protección financiera recomendada.`;
   ...
   */
  private evaluarExpresionSegura(
    expr: string,
    ...
    if (e.includes("&&")) {
      return e.split("&&").every((part) =>
        this.evaluarExpresionSegura(part.trim(), ctx)
      );
    }
    if (e.includes("||")) {
      return e.split("||").some((part) =>
        this.evaluarExpresionSegura(part.trim(), ctx)
      );
    }
    ...
    for (const op of operadores) {
      ...
      switch (op) {
        case "===":
          return valorCtx === valorLiteral;
        case "!==":
          return valorCtx !== valorLiteral;
        case ">=":
          return (valorCtx as number) >= (valorLiteral as number);
        case "<=":
          return (valorCtx as number) <= (valorLiteral as number);
        case ">":
          return (valorCtx as number) > (valorLiteral as number);
  ...
  calcularRiesgo(clausulaIds: string[]): {
    ...
    const riesgoPromedio = clausulaIds.length > 0
      ? riesgoTotal / clausulaIds.length
      : 0;
    const nivelRiesgo: "bajo" | "medio" | "alto" = riesgoPromedio <= 3
      ? "bajo"
      : riesgoPromedio <= 6
      ? "medio"
      : "alto";
    ...
    if (nivelRiesgo === "alto") {
      recomendaciones.push(
        "Revisar con departamento legal antes de enviar al cliente.",
      );
      recomendaciones.push(
        "Considerar incluir cláusula de garantía adicional.",
      );
    }
  ...
  actualizar(id: string, datos: Partial<ClausulaLegal>): ClausulaLegal | null {
    ...
    if (
      datos.contenido && datos.contenido !== this.clausulas[index].contenido
    ) {
...
export function useClausulas() {
  ...
  return {
    ...
    obtenerVariablesPendientes: service.obtenerVariablesPendientes.bind(
      service,
    ),
