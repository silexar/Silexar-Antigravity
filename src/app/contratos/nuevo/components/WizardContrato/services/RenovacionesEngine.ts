...
export interface AccionRenovacion {
  tipo: 'contactar' | 'reunion' | 'propuesta' | 'intervencion_gerencial' | 'oferta_defensiva';
  descripcion: string;
  diasAntes: number;
...
export interface AnalisisPerformance {
  kpisAlcanzados: { nombre: string; objetivo: number; logrado: number; porcentaje: number }[];
  roi: number;
...
const mockContratosRenovacion: ContratoRenovacion[] = [
  {
    ...
    factoresAnalisis: [
      { nombre: 'Performance Campaña', valor: 95, impacto: 'positivo', peso: 25, descripcion: 'KPIs superados en 15%' },
      { nombre: 'Satisfacción (NPS)', valor: 85, impacto: 'positivo', peso: 20, descripcion: 'NPS 8.5/10' },
      { nombre: 'Historial Renovaciones', valor: 100, impacto: 'positivo', peso: 20, descripcion: '3 renovaciones consecutivas' },
      { nombre: 'Actividad Competencia', valor: 30, impacto: 'positivo', peso: 15, descripcion: 'Sin actividad detectada' },
      { nombre: 'Ciclo Presupuestario', valor: 90, impacto: 'positivo', peso: 20, descripcion: 'Presupuesto aprobado para 2025' }
    ],
    accionRecomendada: {
      tipo: 'contactar',
  ...
  },
  {
    ...
    nivelProbabilidad: 'ALTA',
    factoresAnalisis: [
      { nombre: 'Performance Campaña', valor: 88, impacto: 'positivo', peso: 25, descripcion: 'KPIs alcanzados' },
      { nombre: 'Satisfacción (NPS)', valor: 80, impacto: 'positivo', peso: 20, descripcion: 'NPS 8/10' },
      { nombre: 'Historial Renovaciones', valor: 100, impacto: 'positivo', peso: 20, descripcion: '5 años de relación' },
      { nombre: 'Actividad Competencia', valor: 50, impacto: 'neutro', peso: 15, descripcion: 'Cotizaciones solicitadas' },
      { nombre: 'Ciclo Presupuestario', valor: 85, impacto: 'positivo', peso: 20, descripcion: 'En planificación' }
    ],
    accionRecomendada: {
      tipo: 'propuesta',
  ...
  },
  {
    ...
    factoresAnalisis: [
      { nombre: 'Performance Campaña', valor: 70, impacto: 'neutro', peso: 25, descripcion: 'KPIs parcialmente alcanzados' },
      { nombre: 'Satisfacción (NPS)', valor: 65, impacto: 'neutro', peso: 20, descripcion: 'NPS 6.5/10' },
      { nombre: 'Historial Renovaciones', valor: 50, impacto: 'neutro', peso: 20, descripcion: 'Primera renovación' },
      { nombre: 'Actividad Competencia', valor: 70, impacto: 'negativo', peso: 15, descripcion: 'Competidor activo' },
      { nombre: 'Ciclo Presupuestario', valor: 60, impacto: 'neutro', peso: 20, descripcion: 'Revisión de gastos' }
    ],
    accionRecomendada: {
      tipo: 'reunion',
  ...
  },
  {
    ...
    factoresAnalisis: [
      { nombre: 'Performance Campaña', valor: 55, impacto: 'negativo', peso: 25, descripcion: 'KPIs no alcanzados' },
      { nombre: 'Satisfacción (NPS)', valor: 50, impacto: 'negativo', peso: 20, descripcion: 'NPS 5/10 - problemas reportados' },
      { nombre: 'Historial Renovaciones', valor: 100, impacto: 'positivo', peso: 20, descripcion: '2 renovaciones previas' },
      { nombre: 'Actividad Competencia', valor: 90, impacto: 'negativo', peso: 15, descripcion: 'Pitch activo con competidor' },
      { nombre: 'Cambios Equipo Cliente', valor: 20, impacto: 'negativo', peso: 20, descripcion: 'Nuevo gerente de marketing' }
    ],
    accionRecomendada: {
...
class RenovacionesEngineClass {
  ...
  getContratosProximosVencer(diasAdelante: number = 30): ContratoRenovacion[] {
    const fechaLimite = new Date(Date.now() + diasAdelante * 24 * 60 * 60 * 1000);
    
    return Array.from(this.contratos.values())
      .filter(c => c.fechaVencimiento <= fechaLimite && c.estadoWorkflow !== 'RENOVADO' && c.estadoWorkflow !== 'PERDIDO')
   ...
   */
  getContratosAgrupados(diasAdelante: number = 30): Record<NivelProbabilidad, ContratoRenovacion[]> {
    const contratos = this.getContratosProximosVencer(diasAdelante);
    
    return {
      ALTA: contratos.filter(c => c.probabilidadRenovacion >= 85),
      MEDIA: contratos.filter(c => c.probabilidadRenovacion >= 50 && c.probabilidadRenovacion < 85),
      BAJA: contratos.filter(c => c.probabilidadRenovacion >= 30 && c.probabilidadRenovacion < 50),
  ...
  async ejecutarWorkflowDiario(): Promise<{
    ...
    for (const contrato of contratos) {
      const diasRestantes = this.calcularDiasRestantes(contrato.fechaVencimiento);

      // 30 días antes: Análisis y alerta inicial
      if (diasRestantes <= this.config.diasAlertaInicial && contrato.estadoWorkflow === 'PENDIENTE_ANALISIS') {
        await this.enviarAlertaInicial(contrato);
        alertasEnviadas++;
      }

      // 14 días antes: Escalamiento a supervisor si no hay acción
      if (diasRestantes <= this.config.diasAlertaSupervisor && 
      ...
      if (diasRestantes <= this.config.diasGeneracionPropuesta && 
      ...
      if (diasRestantes <= this.config.diasAlertaCritica && 
  ...
  private async enviarAlertaInicial(contrato: ContratoRenovacion): Promise<void> {
    contrato.estadoWorkflow = 'ALERTA_ENVIADA';
  ...
  private async escalarASupervisor(contrato: ContratoRenovacion): Promise<void> {
  ...
  private async enviarAlertaCritica(_contrato: ContratoRenovacion): Promise<void> {
   ...
   */
  async generarPropuestaAutomatica(contrato: ContratoRenovacion): Promise<PropuestaRenovacion> {
    ...
    const descuentoOptimo = this.calcularDescuentoOptimo(contrato, benchmarkMercado);
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private async analizarPerformanceCampana(_contrato: ContratoRenovacion): Promise<AnalisisPerformance> {
    return {
      kpisAlcanzados: [
        { nombre: 'Alcance', objetivo: 1000000, logrado: 1150000, porcentaje: 115 },
      ...
      areasImprovement: ['Optimizar horarios nocturnos', 'Mejorar material creativo']
    };
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private async obtenerBenchmarkMercado(_contrato: ContratoRenovacion): Promise<BenchmarkMercado> {
    return {
      precioMercadoPromedio: 95000000,
      posicionCompetitiva: 'en_linea',
      competidoresActivos: ['Medio Competidor A', 'Plataforma Digital B'],
      diferenciadoresValor: ['Cobertura nacional', 'Tecnología Cortex', 'Servicio premium']
    };
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private calcularDescuentoOptimo(contrato: ContratoRenovacion, _benchmark: BenchmarkMercado): number {
  ...
  async registrarContacto(contratoId: string, notas: string, resultado: string): Promise<void> {
    const contrato = this.contratos.get(contratoId);
    if (!contrato) throw new Error('Contrato no encontrado');
  ...
  getEstadisticas(): {
    ...
    return {
      ...
      accionesPendientes: contratos.filter(c => 
        c.estadoWorkflow === 'ALERTA_ENVIADA' || c.estadoWorkflow === 'ESCALADO'
      ).length
    };
  }
}
