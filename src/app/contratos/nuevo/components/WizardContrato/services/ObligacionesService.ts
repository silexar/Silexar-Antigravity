  ...
  try {
    ...
    if (stored) {
      return JSON.parse(stored).map((o: ObligacionContrato) => ({
        ...
        fechaCompletada: o.fechaCompletada ? new Date(o.fechaCompletada) : undefined,
...
export class ObligacionesService {
   ...
   */
  crear(params: Omit<ObligacionContrato, 'id' | 'estado' | 'porcentajeCompletado' | 'documentosAdjuntos' | 'alertasEnviadas' | 'creadoPor' | 'fechaCreacion' | 'ultimaModificacion' | 'extraidaPorIA'>): ObligacionContrato {
   ...
   */
  actualizar(id: string, datos: Partial<ObligacionContrato>): ObligacionContrato | null {
  ...
  async extraerDeContrato(contrato: {
    ...
    fechaInicio: Date;
    fechaFin: Date;
    terminosPago: { diasPago: number; modalidad: string; numeroCuotas?: number };
    valorNeto: number;
    esCanje: boolean;
    lineasEspecificacion: { medioNombre: string; fechaInicio: Date; fechaFin: Date }[];
  }): Promise<ObligacionContrato[]> {
    const obligacionesExtraidas: ObligacionContrato[] = [];
    
    // 1. Obligaciones de pago
    if (contrato.terminosPago.modalidad === 'cuotas' && contrato.terminosPago.numeroCuotas) {
      const valorCuota = contrato.valorNeto / contrato.terminosPago.numeroCuotas;
      ...
      for (let i = 0; i < contrato.terminosPago.numeroCuotas; i++) {
        ...
        const obligacion = this.crearObligacionExtraida({
          ...
          descripcion: `Pago de cuota ${i + 1} por valor de $${valorCuota.toLocaleString()}`,
    ...
    const obligacionMaterial = this.crearObligacionExtraida({
      ...
      tipo: 'entrega_material',
      titulo: 'Entrega de Material Creativo',
      descripcion: 'El cliente debe entregar todo el material creativo para la campaña',
      clausulaOrigen: 'Cláusula de Materiales',
      responsable: { tipo: 'cliente' },
      fechaInicio: new Date(),
      fechaLimite: new Date(contrato.fechaInicio.getTime() - 7 * 24 * 60 * 60 * 1000), // 7 días antes
      frecuencia: 'unico',
    ...
    const mesesContrato = Math.ceil(
      (contrato.fechaFin.getTime() - contrato.fechaInicio.getTime()) / (30 * 24 * 60 * 60 * 1000)
    );
    ...
    const obligacionFactura = this.crearObligacionExtraida({
      ...
      fechaInicio: contrato.fechaInicio,
      fechaLimite: new Date(contrato.fechaInicio.getTime() + 5 * 24 * 60 * 60 * 1000),
      frecuencia: 'unico',
  ...
  private crearObligacionExtraida(
    params: Omit<ObligacionContrato, 'id' | 'estado' | 'porcentajeCompletado' | 'documentosAdjuntos' | 'alertasEnviadas' | 'creadoPor' | 'fechaCreacion' | 'ultimaModificacion' | 'extraidaPorIA' | 'confianzaExtraccion'>
  ): ObligacionContrato {
    const obligacion: ObligacionContrato = {
      ...params,
      id: crypto.randomUUID(),
   ...
   */
  marcarCompletada(id: string, evidencia?: { nombre: string; url: string; tipo: string }): ObligacionContrato | null {
   ...
   */
  actualizarProgreso(id: string, porcentaje: number): ObligacionContrato | null {
    const estado: EstadoObligacion = porcentaje >= 100 ? 'completada' : 
                                     porcentaje > 0 ? 'en_progreso' : 'pendiente';
      ...
      if (o.estado === 'pendiente' || o.estado === 'en_progreso') {
        ...
        if (o.diasAnticipacionAlerta.includes(diasRestantes)) {
          const yaEnviada = o.alertasEnviadas.some(a => {
            const diasAlEnviar = Math.ceil(
              (o.fechaLimite.getTime() - a.fecha.getTime()) / (24 * 60 * 60 * 1000)
            );
          ...
          if (!yaEnviada) {
            alertas.push({
              ...
              descripcion: `Quedan ${diasRestantes} día(s) para cumplir con: ${o.descripcion}`,
              urgencia: diasRestantes <= 1 ? 'critica' : diasRestantes <= 3 ? 'alta' : 'media',
        ...
        if (diasRestantes < 0) {
          alertas.push({
            ...
            descripcion: `La obligación venció hace ${Math.abs(diasRestantes)} día(s)`,
            urgencia: 'critica',
            fechaGeneracion: new Date(),
            contratoId: o.contratoId,
   ...
   */
  registrarAlertaEnviada(obligacionId: string, canal: 'email' | 'push' | 'sms' | 'whatsapp', destinatario: string): void {
  ...
  obtenerResumen(contratoId?: string): ResumenObligaciones {
    ...
    obligaciones.forEach(o => {
      ...
      } else if (diasRestantes > 0 && diasRestantes <= 7 && o.estado !== 'completada') {
        proximasVencer++;
        if (diasRestantes <= 3) {
          obligacionesCriticas.push(o);
        }
    ...
    const cumplimientoPorcentaje = total > 0 ? (completadas / total) * 100 : 100;
