...
export interface WizardContratoState {
  ...
  configuracionAntiFraude: {
    evidenciasSubidas: Array<{
      id: string;
      tipo: 'email_cliente' | 'orden_compra' | 'cotizacion_firmada' | 'whatsapp_chat' | 'grabacion_llamada' | 'minuta_reunion' | 'otro';
      nombre: string;
      url: string;
      fechaSubida: Date;
    ...
    estado: 'borrador' | 'pendiente_evidencia' | 'pendiente_aprobacion' | 'aprobado_parcial' | 'pendiente_reaprobacion' | 'operativo' | 'rechazado' | 'suspendido';
...
export type WizardAction =
  ...
  | { type: 'SET_PROPIEDADES'; payload: WizardContratoState['propiedadesSeleccionadas'] }
  ...
  | { type: 'SET_COMISION_AGENCIA'; payload: { comision: number; facturar: boolean } }
  
  // Paso 3
  | { type: 'ADD_LINEA_ESPECIFICACION'; payload: EspecificacionPauta }
  | { type: 'UPDATE_LINEA_ESPECIFICACION'; payload: { id: string; data: Partial<EspecificacionPauta> } }
  ...
  | { type: 'UPDATE_APROBADOR'; payload: { id: string; estado: AprobadorContrato['estado'] } }
  ...
  | { type: 'UPDATE_DOCUMENTO'; payload: { id: string; data: Partial<DocumentoContrato> } }
  | { type: 'SET_FIRMA_DIGITAL'; payload: boolean }
  | { type: 'SET_URL_FIRMA'; payload: string }
  
  // Paso 6: Anti-Fraude
  | { type: 'SET_CONFIGURACION_ANTI_FRAUDE'; payload: WizardContratoState['configuracionAntiFraude'] }
  | { type: 'ADD_EVIDENCIA_NEGOCIACION'; payload: WizardContratoState['configuracionAntiFraude']['evidenciasSubidas'][0] }
  | { type: 'REMOVE_EVIDENCIA_NEGOCIACION'; payload: string }
  | { type: 'VALIDAR_EVIDENCIA'; payload: { id: string; validadoPor: { id: string; nombre: string; fecha: Date } } }
  | { type: 'SET_ESTADO_ANTI_FRAUDE'; payload: WizardContratoState['configuracionAntiFraude']['estado'] }
...
export interface StepConfig {
  ...
  descripcion: string;
  icono: string;
  validacion: (state: WizardContratoState) => { isValid: boolean; errors: string[] };
  esOpcional: boolean;
}
...
export const WIZARD_STEPS: StepConfig[] = [
  {
    ...
    validacion: (state) => {
      ...
      if (!state.campana.trim()) errors.push('El nombre de campaña es requerido');
      ...
      if (state.fechaInicio && state.fechaFin && state.fechaFin <= state.fechaInicio) {
        errors.push('La fecha de fin debe ser posterior a la de inicio');
      }
      if (!state.propiedadesSeleccionadas || state.propiedadesSeleccionadas.length === 0) {
        errors.push('Debe clasificar el contrato con al menos una propiedad maestra (ej. Tipo de Creatividad).');
      }
  ...
  },
  {
    ...
    descripcion: 'Análisis de riesgo y condiciones de pago',
    icono: '💰',
    validacion: (state) => {
      const errors: string[] = [];
      if (state.valorBruto <= 0) errors.push('El valor del contrato debe ser mayor a 0');
      if (state.terminosPago.diasPago <= 0) errors.push('Los días de pago deben ser mayores a 0');
      return { isValid: errors.length === 0, errors };
    },
    esOpcional: false
  },
  {
    ...
    validacion: (state) => {
      ...
      const sinValidar = state.lineasEspecificacion.filter(
        l => !l.validacionInventario || l.validacionInventario.estado === 'no_disponible'
      );
      if (sinValidar.length > 0) {
        errors.push('Hay líneas con problemas de inventario');
      }
      return { isValid: errors.length === 0, errors };
    },
    esOpcional: false
  },
  {
    ...
    validacion: (state) => {
      const errors: string[] = [];
      if (!state.flujoAprobacion) errors.push('Debe configurar el flujo de aprobación');
      return { isValid: errors.length === 0, errors };
    },
    esOpcional: false
  },
  ...
  },
  {
    ...
    validacion: (state) => {
      ...
      const evidenciasValidadas = config.evidenciasSubidas.filter(e => e.validado);
      if (config.evidenciasSubidas.length > 0 && evidenciasValidadas.length === 0) {
        errors.push('Al menos una evidencia debe estar validada');
      }
      
      // Validar justificación para descuentos altos
      if (state.descuentoPorcentaje >= 65 && !config.justificacionDescuento?.texto) {
...
export const calcularValorNeto = (bruto: number, descuento: number): number => {
...
export const determinarNivelAprobacion = (
  ...
  if (valor < 10000000 && descuento <= 10 && diasPago <= 30 && !esNuevoCliente) {
    return 'automatico';
  }
...
export const formatCurrency = (value: number, moneda: Moneda = 'CLP'): string => {
  const formatters: Record<Moneda, Intl.NumberFormat> = {
    ...
    UF: new Intl.NumberFormat('es-CL', { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 2 })
  };
