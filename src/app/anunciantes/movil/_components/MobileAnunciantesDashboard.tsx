    ...
    <div className="space-y-6 pb-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-between items-center mb-2">
        <div>
          ...
          <p className="text-xs text-slate-500">Métricas de anunciantes en tiempo real</p>
        </div>
        <button 
          onClick={onRefresh}
          className={`p-2.5 rounded-full bg-white/60 backdrop-blur-sm border border-white/60 shadow-sm active:scale-95 text-blue-500 ${loading ? 'animate-spin text-slate-400' : ''}`}
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 rounded-3xl p-5 bg-gradient-to-br from-blue-500 to-blue-600 shadow-lg shadow-blue-200/50 border border-white/20 relative overflow-hidden">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-white/10 rounded-full blur-xl"></div>
          <div className="flex justify-between items-start relative z-10">
            <div>
              <p className="text-blue-100 text-sm font-medium mb-1">Total Anunciantes</p>
        ...
        <div className="rounded-3xl p-5 bg-white/60 backdrop-blur-xl shadow-sm border border-white/60">
          ...
          <h4 className="text-2xl font-bold text-slate-800">{stats.activos}</h4>
        ...
        <div className="rounded-3xl p-5 bg-white/60 backdrop-blur-xl shadow-sm border border-white/60">
          ...
          <h4 className="text-2xl font-bold text-slate-800">{stats.inactivos}</h4>
          <p className="text-xs text-slate-500 font-medium mt-1">Inactivos</p>
        </div>
      </div>

      {/* Acciones Rápidas */}
      <h3 className="text-lg font-bold text-slate-800 mt-8 mb-4">Acciones Rápidas</h3>
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4">
        {[
          { icon: Plus, label: "Nuevo", color: "text-blue-600", bg: "bg-blue-100", action: onOpenNuevo },
          { icon: Search, label: "Buscar", color: "text-purple-600", bg: "bg-purple-100", action: onOpenList },
          { icon: Filter, label: "Filtrar", color: "text-emerald-600", bg: "bg-emerald-100", action: onOpenList },
        ].map((btn, idx) => (
          <button 
            key={idx}
            onClick={btn.action}
      ...
      <div className="flex justify-between items-end mt-4 mb-4">
        <h3 className="text-lg font-bold text-slate-800">Agregados Recientemente</h3>
        <button onClick={onOpenList} className="text-sm font-semibold text-blue-600">Ver todos</button>
      </div>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="p-6 text-center rounded-3xl bg-slate-50 border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 text-sm">No hay anunciantes recientes</p>
          </div>
        ) : (
          recentAnunciantes.map(anunciante => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-slate-800 truncate">{anunciante.nombreRazonSocial}</h4>
                <p className="text-xs text-slate-500 truncate mt-0.5">{anunciante.codigo} • {anunciante.rut}</p>
              </div>
