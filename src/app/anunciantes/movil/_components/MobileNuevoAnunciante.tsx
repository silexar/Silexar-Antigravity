...
export const MobileNuevoAnunciante: React.FC<CrearProps> = ({ onBack }) => {
  ...
  const StepIndicator = () => (
    <div className="flex items-center justify-between px-6 py-4 bg-white sticky top-0 z-10 border-b border-slate-100 shadow-sm">
      {[
        ...
        <div key={s.id} className="flex flex-col items-center">
          ...
          <span className={`text-[9px] font-extrabold uppercase tracking-wide ${step === s.id ? 'text-blue-600' : 'text-slate-400'}`}>{s.label}</span>
        </div>
      ))}
    </div>
  );
  ...
  const renderInput = (name: keyof typeof formData, placeholder: string, type = 'text') => (
    <input
      ...
      className="w-full text-base py-3.5 px-4 bg-white border border-slate-200 rounded-2xl shadow-[inset_2px_2px_4px_rgba(0,0,0,0.02)] focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder:text-slate-400 text-slate-800 font-medium"
    />
  );
  ...
  if (success) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
        ...
        <p className="text-slate-500 mt-2 font-medium">El cliente se ha registrado exitosamente en Silexar Pulse.</p>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 flex flex-col animate-in slide-in-from-bottom-full duration-300">
      ...
      <header className="px-4 py-4 flex items-center gap-4 bg-white shadow-sm z-20">
        <button onClick={step === 1 ? onBack : () => setStep(step - 1)} className="p-2 rounded-full active:bg-slate-100 text-slate-700">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-bold text-slate-800 flex-1">Nuevo Anunciante</h1>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-5 pb-32">
        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {step === 1 && (
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">Datos de la Empresa</h2>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">Ubicación</h2>
          ...
          {step === 3 && (
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">Contacto Principal</h2>
          ...
          {step === 4 && (
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">Ajustes Finales</h2>
              
              <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-slate-700">Factura Electrónica (DTE)</span>
                  <button 
                    onClick={() => setFormData(p => ({ ...p, tieneFacturacionElectronica: !p.tieneFacturacionElectronica }))}
                    className={`w-14 h-8 rounded-full flex items-center p-1 transition-colors ${formData.tieneFacturacionElectronica ? 'bg-blue-600' : 'bg-slate-300'}`}
                  >
                ...
                <p className="text-xs text-slate-500 font-medium leading-relaxed">Active si el anunciante está habilitado en SII para recepción de DTE.</p>
              </div>
              ...
              <InputWrapper label="Notas Internas">
                <textarea
                  ...
                  className="w-full text-base py-3 px-4 bg-white border border-slate-200 rounded-2xl shadow-[inset_2px_2px_4px_rgba(0,0,0,0.02)] focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder:text-slate-400 text-slate-800 font-medium resize-none"
                />
              </InputWrapper>
            </div>
          )}
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md p-4 border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] pb-safe">
        {step < 4 ? (
          <button 
