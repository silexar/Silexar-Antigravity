...
export async function GET(request: NextRequest) {
  ...
  if (!checkPermission(ctx, 'usuarios', 'read')) return apiForbidden();
...
export async function POST(request: NextRequest) {
  ...
  // 4. Check permissions (RBAC) — creating users requires manage permission
  if (!checkPermission(ctx, 'usuarios', 'admin')) return apiForbidden();
...
export async function PATCH(request: NextRequest) {
  ...
  // 4. Check permissions (RBAC) — only managers+ can check permissions of other users
  if (!checkPermission(ctx, 'usuarios', 'read')) return apiForbidden();
