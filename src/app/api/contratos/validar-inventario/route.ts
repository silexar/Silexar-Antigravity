...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('[API/Contratos/ValidarInventario] Error:', error instanceof Error ? error : undefined, { module: 'validar-inventario' });
    return NextResponse.json(
      { success: false, error: 'Error validando inventario' },
      { status: 500 }
    );
  }
}
