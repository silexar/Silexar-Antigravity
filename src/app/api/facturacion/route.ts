...
export async function GET(request: NextRequest) {
  try {
    ...
    const perm = checkPermission(ctx, 'facturacion', 'read');
    ...
    await auditLogger.logEvent({ type: 'DATA_READ', userId: ctx.userId, module: 'facturacion' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const perm = checkPermission(ctx, 'facturacion', 'update');
    if (!perm) return apiForbidden();
