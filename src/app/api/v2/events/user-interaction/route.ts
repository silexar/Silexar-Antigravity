...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return apiSuccess({ eventId: `evt_${Date.now()}`, type: 'USER_INTERACTION', ...body });
  } catch (error) {
    logger.error('[API/V2/Events/UserInteraction] Error:', error instanceof Error ? error : undefined);
