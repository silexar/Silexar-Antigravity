...
export async function POST(request: Request) {
  // SIMULACIÓN RBAC TIER 0: Verificar que el usuario tenga rol de Tráfico o Admin
  const userRole = request.headers.get('x-user-role'); 
  if (userRole !== 'TRAFFIC' && userRole !== 'ADMIN') {
      return NextResponse.json({ error: 'Acceso Denegado: Se requiere rol de Tráfico' }, { status: 403 });
  }
