...
export function WizardCrearPropiedad({ onClose, onComplete }: WizardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#F0EDE8]/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col h-[90vh] max-h-[850px] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-[#888780] flex items-center gap-2">
            ...
            {[
              ...
              <div key={s.num} className="relative z-10 flex flex-col items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-colors ${
                  step > s.num ? 'bg-indigo-500 border-indigo-500 text-[#2C2C2A]' :
                  step === s.num ? 'bg-white border-indigo-500 text-indigo-600 shadow-sm' :
                  'bg-white border-slate-200 text-[#5F5E5A]'
                }`}>
                  {step > s.num ? <Check className="w-5 h-5" /> : s.icon}
                </div>
                <span className={`text-xs font-bold uppercase tracking-wider ${
                  step >= s.num ? 'text-slate-700' : 'text-[#888780]'
                }`}>{s.title}</span>
              </div>
            ))}
          </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      >
                        <div className="w-10 h-10 rounded-lg bg-slate-50 text-[#888780] flex items-center justify-center mb-3 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                          {t.icon}
                        </div>
                        <h4 className="font-bold text-slate-700 mb-1">{t.title}</h4>
                        <p className="text-xs text-[#888780]">{t.desc}</p>
                      </button>
                    ))}
                  </div>
                </>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-[#888780]">Defina la identidad básica de esta nueva clasificación.</p>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {['Tipo de Creatividad', 'Canal de Distribución', 'Formato Publicitario'].map(sug => (
                            <button 
                              ...
                              className="px-3 py-1 bg-white border border-indigo-200 text-indigo-600 text-xs rounded-full hover:bg-indigo-600 hover:text-[#2C2C2A] transition-colors"
                            >
                              "{sug}"
                            </button>
                          ))}
                    ...
                    <div className="pt-2">
                      <label className="text-sm font-bold text-slate-700 uppercase tracking-wider text-[11px] mb-3 block flex items-center gap-2">
                        <Target className="w-4 h-4 text-[#888780]" />
                          ...
                          return (
                            <button
                              ...
                              className={`py-2 px-3 rounded-lg border text-sm font-medium flex items-center gap-2 transition-colors ${
                                isSelected ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-[#888780] hover:bg-slate-50'
                              }`}
                            >
                              <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${isSelected ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'}`}>
                                {isSelected && <Check className="w-2.5 h-2.5 text-[#2C2C2A]" />}
                              </div>
                              {app}
                            </button>
                          );
          ...
          {step === 2 && (
            <div className="max-w-3xl mx-auto animate-in slide-in-from-right-8 duration-300 space-y-6">
              <div>
                ...
                <p className="text-sm text-[#888780]">Defina las reglas y restricciones para los valores de esta propiedad.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-4 flex items-center gap-2"><ListTree className="w-3.5 h-3.5"/> Tipo de Selección</h4>
                  <div className="space-y-2">
                    {[
                      ...
                      <label key={opt.id} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${formData.tipoSeleccion === opt.id ? 'bg-indigo-50/50 border-indigo-200' : 'border-slate-100 hover:bg-slate-50'}`}>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-[#888780]">{opt.desc}</div>
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><CheckCircle2 className="w-3.5 h-3.5"/> Validaciones Básicas</h4>
                ...
                {/* Gestion Conflictos */}
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><AlertTriangle className="w-3.5 h-3.5"/> Gestión de Conflictos</h4>
                ...
                <div className="space-y-6">
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><Filter className="w-3.5 h-3.5"/> Configuración en Reportes</h4>
          ...
          {step === 3 && (
            <div className="max-w-3xl mx-auto animate-in slide-in-from-right-8 duration-300 space-y-6">
              <div>
                ...
                <p className="text-sm text-[#888780]">Vincule esta propiedad con el impacto financiero y los centros de costos.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  {/* Impacto */}
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-4 flex items-center gap-2"><Target className="w-3.5 h-3.5"/> Tipo de Impacto Contable</h4>
                    <div className="space-y-2">
                      {[
                  ...
                  {/* Defecto */}
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><Settings className="w-3.5 h-3.5"/> Configuración por Defecto</h4>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] font-bold text-[#888780] uppercase">Cta. Ingresos</label>
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-[#888780] uppercase">Cta. Costos</label>
                        <select className="w-full mt-1 bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm font-mono text-slate-700">
                          <option>5110000</option>
                        </select>
                      </div>
                      <div className="col-span-2">
                        <label className="text-[10px] font-bold text-[#888780] uppercase">Centro de Costo</label>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><ShieldAlert className="w-3.5 h-3.5"/> Validaciones Financieras</h4>
          ...
          {step === 4 && (
            <div className="max-w-3xl mx-auto animate-in slide-in-from-right-8 duration-300 space-y-6">
              <div>
                ...
                <p className="text-sm text-[#888780]">Defina la estructura semilla de datos o utilice la propuesta de la IA.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-1.5 bg-white p-2 rounded-lg border border-indigo-50/50">
                      {formData.valoresSugeridos.map(v => (
                        <label key={v.id} className="flex items-center gap-3 p-2 rounded hover:bg-slate-50 cursor-pointer transition-colors border border-transparent hover:border-slate-100">
                          ...
                          <div className="font-mono text-xs text-[#888780] w-5">{v.id}</div>
                          <div className={`text-xs font-bold ${v.checked ? 'text-slate-700' : 'text-[#888780]'}`}>{v.desc}</div>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><Plus className="w-3.5 h-3.5"/> Agregar Valor Manual</h4>
                ...
                <div className="space-y-4 flex flex-col">
                  ...
                  <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-2 flex items-center gap-2"><Settings className="w-3.5 h-3.5"/> Configuración Rápida</h4>
                  ...
                  <div className="bg-[#E8E5E0] rounded-xl p-5 shadow-md border border-[#D4D1CC] flex-1 relative overflow-hidden flex flex-col">
                    <div className="absolute top-0 right-0 p-3 opacity-10">
                      <FolderTree className="w-24 h-24 text-[#2C2C2A]" />
                    </div>
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[#888780] mb-4 flex items-center gap-2 z-10"><Eye className="w-3.5 h-3.5"/> Preview Estructura</h4>
                    
                    <div className="font-mono text-[11px] text-[#5F5E5A] z-10 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                      <div className="font-bold text-indigo-300 mb-2 text-xs">{formData.nombre || 'TIPO PROPIEDAD'}</div>
                      <div className="space-y-1.5 border-l border-[#CCCAC5] ml-1.5 pl-3">
                        ...
                        {formData.valoresSugeridos.filter(v => v.checked).length === 0 && (
                          <div className="text-[#888780] italic">Estructura vacía...</div>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-white flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button 
              onClick={onClose}
              className="px-4 py-2 text-[#888780] hover:bg-slate-100 font-medium rounded-lg text-sm transition-colors"
            >
              Cancelar
            </button>
            ...
            {step < 4 ? (
              <button 
                onClick={handleNext}
                disabled={!formType}
                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-[#2C2C2A] font-medium rounded-lg text-sm flex items-center gap-2 shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button 
                onClick={finish}
                className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-[#2C2C2A] font-bold rounded-lg text-sm flex items-center gap-2 shadow-md shadow-emerald-200 transition-colors"
              >
    ...
    <div className="flex items-center justify-between group">
      ...
      >
        {checked ? (
          ...
          <ToggleLeft className="w-8 h-8 text-[#5F5E5A] transition-colors" />
        )}
      </button>
    </div>
  );
