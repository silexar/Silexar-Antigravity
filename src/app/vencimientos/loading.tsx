        ...
        <div className="max-w-[1920px] mx-auto px-6 py-4">
          ...
          <div className="flex gap-2 mt-4">
            {Array.from({ length: 7 }).map((_, i) => (
              <div key={`${_}-${i}`} className="h-9 w-24 bg-gray-200 rounded-full animate-pulse" />
            ))}
          </div>
        </div>
      </header>
      <main className="max-w-[1920px] mx-auto px-6 py-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={`${_}-${i}`} className="h-28 rounded-2xl bg-white/70 border border-white/40 shadow-lg shadow-gray-200/50 animate-pulse" />
          ))}
        </div>
