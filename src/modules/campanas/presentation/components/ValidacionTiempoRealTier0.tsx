...
export const ValidacionTiempoRealTier0: React.FC<{ validations?: ValidationResult[] }> = ({ validations = [] }) => (
    ...
    <CardContent className="space-y-2">{validations.map((v, i) => (
        <div key={i} className="flex items-center gap-2">{v.valid ? <CheckCircle className="w-4 h-4 text-green-500" /> : <AlertCircle className="w-4 h-4 text-red-500" />}<span>{v.field}</span>{v.message && <span className="text-sm text-gray-500">- {v.message}</span>}</div>
    ))}</CardContent></Card>
);
