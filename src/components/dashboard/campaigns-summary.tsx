...
export const CampaignsSummary: React.FC<{ className?: string }> = ({ className = '' }) => (
    <Card className={className}>
        ...
        <CardContent>{campaigns.map((c, i) => <div key={i} className="p-2 border-b last:border-0"><span className="font-medium">{c.name}</span> - {c.status} ({c.roi}% ROI)</div>)}</CardContent>
    </Card>
);
